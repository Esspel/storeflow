...
CREATE POLICY IF NOT EXISTS "shelf_life_store_isolation"
...
CREATE POLICY IF NOT EXISTS "shelf_life_insert_for_store"
  ON product_shelf_life
  FOR INSERT
  TO authenticated
...
CREATE POLICY IF NOT EXISTS "shelf_life_update_for_store"
  ON product_shelf_life
  FOR UPDATE
  TO authenticated
  USING (store_id = current_setting('app.current_store_id')::text);
...
CREATE POLICY IF NOT EXISTS "reclamation_history_store_isolation"
...
CREATE POLICY IF NOT EXISTS "reclamation_history_insert_for_store"
  ON product_reclamation_history
  FOR INSERT
  TO authenticated
  WITH CHECK (store_id = current_setting('app.current_store_id')::text);
...
CREATE POLICY IF NOT EXISTS "delivery_log_store_isolation"
...
CREATE POLICY IF NOT EXISTS "delivery_log_insert_for_store"
  ON product_delivery_log
  FOR INSERT
  TO authenticated
  WITH CHECK (store_id = current_setting('app.current_store_id')::text);
