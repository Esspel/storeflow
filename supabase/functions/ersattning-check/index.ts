...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { sap_article_id, shelf_lifetime_days, expiry_date, arrival_date, compensation_price_ore, store_id } = body
    ...
    if (existing) {
      const { error } = await supabase.from('product_shelf_life').update(payload).eq('id', existing.id).eq('store_id', targetStore)
