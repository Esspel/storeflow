...
export async function updateTemplate(supabase: SupabaseClient, ctx: ApiKeyContext, input: UpdateTemplateInput) {
  requireScope(ctx, "templates:write");
  ...
  const { error: updateErr } = await supabase.from("checklist_templates").update(updates).eq("id", input.template_id);
  ...
  if (Array.isArray(input.store_ids)) {
    ...
    if (input.store_ids.length > 0) {
      const storeRows = input.store_ids.map((store_id) => ({ template_id: input.template_id, store_id }));
      const { error: sErr } = await supabase.from("template_stores").insert(storeRows);
      if (sErr) throw new ScopeError(`Uppdaterade mall men butikskoppling misslyckades: ${sErr.message}`, 500);
    }
  }
  ...
  if (Array.isArray(input.items)) {
    ...
    if (input.items.length > 0) {
      ...
      if (iErr) throw new ScopeError(`Uppdaterade mall men steg misslyckades: ${iErr.message}`, 500);
    }
  }
...
export async function listTasks(
  supabase: SupabaseClient, ctx: ApiKeyContext,
  filters: { store_id?: string; status?: string; category?: string; assigned_to?: string; due_date?: string; limit?: number } = {},
) {
  requireScope(ctx, "tasks:read");
  if (filters.store_id) requireStore(ctx, filters.store_id);

  let query = supabase.from("tasks")
    .select("id, title, description, category, priority, status, store_id, assigned_to, created_by, due_date, due_date_time, completed_at, sap_article_id, created_at")
...
export async function getTask(supabase: SupabaseClient, ctx: ApiKeyContext, taskId: string) {
  ...
  const { data: steps } = await supabase.from("task_steps")
    .select("id, label, is_done, requires_photo, sort_order")
    .eq("task_id", taskId)
    .order("sort_order");

  const { data: questions } = await supabase.from("task_questions")
...
export async function createTask(supabase: SupabaseClient, ctx: ApiKeyContext, input: CreateTaskInput) {
  ...
  if (input.steps && input.steps.length > 0) {
    ...
    if (stepErr) throw new ScopeError(`Uppgift skapad men steg misslyckades: ${stepErr.message}`, 500);
  }
...
export async function updateTask(supabase: SupabaseClient, ctx: ApiKeyContext, input: UpdateTaskInput) {
  requireScope(ctx, "tasks:write");
  ...
  if (Object.keys(updates).length > 0) {
    const { error: updateErr } = await supabase.from("tasks").update(updates).eq("id", input.task_id);
    if (updateErr) throw new ScopeError(`Misslyckades att uppdatera uppgift: ${updateErr.message}`, 500);
  }
    ...
    for (const st of input.steps) {
      if (st.id) {
        ...
        if (Object.keys(stepUpdates).length > 0) {
          await supabase.from("task_steps").update(stepUpdates).eq("id", st.id).eq("task_id", input.task_id);
        }
      }
    }
  }
...
export async function listCustomerRequests(
  supabase: SupabaseClient, ctx: ApiKeyContext,
  filters: { store_id?: string; status?: string; priority?: string; query?: string; limit?: number } = {},
) {
  requireScope(ctx, "customer_requests:read");
  if (filters.store_id) requireStore(ctx, filters.store_id);

  let query = supabase.from("customer_requests")
    .select("id, store_id, product_name, article_number, notes, internal_notes, staff_comment, source, requested_by, status, priority, mitt_coop_category_id, mitt_coop_status_code, created_at")
    .order("created_at", { ascending: false })
    .limit(Math.min(filters.limit ?? 50, 200));
  ...
  if (filters.query) {
    query = query.or(`product_name.ilike.%${filters.query}%,notes.ilike.%${filters.query}%,article_number.ilike.%${filters.query}%`);
  }
...
export async function getCustomerRequest(supabase: SupabaseClient, ctx: ApiKeyContext, requestId: string) {
  requireScope(ctx, "customer_requests:read");
  const { data: request, error } = await supabase.from("customer_requests").select("*").eq("id", requestId).maybeSingle();
  if (error) throw new ScopeError(error.message, 500);
...
export async function createCustomerRequest(supabase: SupabaseClient, ctx: ApiKeyContext, input: CreateCustomerRequestInput) {
  ...
  const { data: request, error } = await supabase.from("customer_requests").insert({
  ...
  if (error || !request) throw new ScopeError(error?.message ?? "Kunde inte skapa kundönskemål.", 500);
  return request;
}
...
export async function updateCustomerRequest(supabase: SupabaseClient, ctx: ApiKeyContext, input: UpdateCustomerRequestInput) {
  requireScope(ctx, "customer_requests:write");
  ...
  const { error } = await supabase.from("customer_requests").update(updates).eq("id", input.request_id);
...
export async function listCustomerRounds(
  ...
  let query = supabase.from("kundrunda_sessions")
    .select("id, store_id, conducted_by, started_at, completed_at, status, total_score, max_score, version, created_at")
...
export async function getCustomerRound(supabase: SupabaseClient, ctx: ApiKeyContext, sessionId: string) {
  requireScope(ctx, "customer_rounds:read");
  const { data: session, error } = await supabase.from("kundrunda_sessions").select("*").eq("id", sessionId).maybeSingle();
  if (error) throw new ScopeError(error.message, 500);
  if (!session) throw new ScopeError("Kundrundan hittades inte.", 404);

  if (session.store_id) requireStore(ctx, session.store_id);

  const { data: responses } = await supabase.from("kundrunda_responses")
    .select("id, session_id, checkpoint_id, zone_id, result, defect_description, action_taken, responsible_user_id, sap_article_id, created_task_id, incident_id, created_at")
...
export async function listDeviations(
  supabase: SupabaseClient, ctx: ApiKeyContext,
  filters: { store_id?: string; status?: string; priority?: string; category?: string; query?: string; limit?: number } = {},
) {
  requireScope(ctx, "deviations:read");
  if (filters.store_id) requireStore(ctx, filters.store_id);

  let query = supabase.from("incidents")
    .select("id, ref_number, title, description, category, store_id, reported_by, assigned_to, responsible_user_id, priority, status, sla_deadline, resolved_at, has_photo, sap_article_id, source, created_at")
    .order("created_at", { ascending: false })
    .limit(Math.min(filters.limit ?? 50, 200));
  ...
  if (filters.category) query = query.eq("category", filters.category);
  if (filters.query) {
    query = query.or(`title.ilike.%${filters.query}%,description.ilike.%${filters.query}%,ref_number.ilike.%${filters.query}%`);
  }
...
export async function getDeviation(supabase: SupabaseClient, ctx: ApiKeyContext, deviationId: string) {
  requireScope(ctx, "deviations:read");
  const { data: incident, error } = await supabase.from("incidents").select("*").eq("id", deviationId).maybeSingle();
  if (error) throw new ScopeError(error.message, 500);
  if (!incident) throw new ScopeError("Avvikelsen hittades inte.", 404);

  if (incident.store_id) requireStore(ctx, incident.store_id);

  const { data: comments } = await supabase.from("incident_comments")
...
export async function createDeviation(supabase: SupabaseClient, ctx: ApiKeyContext, input: CreateDeviationInput) {
  ...
  if (error || !incident) throw new ScopeError(error?.message ?? "Kunde inte skapa avvikelse.", 500);
  return incident;
}
...
export async function updateDeviation(supabase: SupabaseClient, ctx: ApiKeyContext, input: UpdateDeviationInput) {
  requireScope(ctx, "deviations:write");
  ...
  if (input.status !== undefined) {
    updates.status = input.status;
    if ((input.status === "resolved" || input.status === "closed") && !input.resolved_at && !existing.resolved_at) {
      updates.resolved_at = new Date().toISOString();
    }
  }
  if (input.assigned_to !== undefined) updates.assigned_to = input.assigned_to;
  if (input.responsible_user_id !== undefined) updates.responsible_user_id = input.responsible_user_id;
...
export async function listStores(
  ...
  if (ctx.storeId) {
    ...
    if (filters.query) {
      query = query.or(`name.ilike.%${filters.query}%,city.ilike.%${filters.query}%,butiks_nr.ilike.%${filters.query}%,koncept.ilike.%${filters.query}%`);
    }
...
export async function getStore(supabase: SupabaseClient, ctx: ApiKeyContext, storeId: string) {
  ...
  const { data: store, error } = await supabase.from("stores").select("*").eq("id", storeId).maybeSingle();
  if (error) throw new ScopeError(error.message, 500);
...
export async function listTemplatePackages(
  ...
  let query = supabase.from("template_packages")
...
export async function getTemplatePackage(supabase: SupabaseClient, ctx: ApiKeyContext, packageId: string) {
  requireScope(ctx, "template_packages:read");
  const { data: pkg, error } = await supabase.from("template_packages").select("*").eq("id", packageId).maybeSingle();
  if (error) throw new ScopeError(error.message, 500);
  if (!pkg) throw new ScopeError("Mallpaketet hittades inte.", 404);

  if (pkg.store_id) requireStore(ctx, pkg.store_id);

  const { data: items } = await supabase.from("template_package_items")
...
export async function createTemplatePackage(supabase: SupabaseClient, ctx: ApiKeyContext, input: CreateTemplatePackageInput) {
  ...
  const { data: pkg, error } = await supabase.from("template_packages").insert({
  ...
  if (input.template_ids && input.template_ids.length > 0) {
    ...
    if (itemErr) throw new ScopeError(`Mallpaket skapat men mallkopplingar misslyckades: ${itemErr.message}`, 500);
  }
...
export async function updateTemplatePackage(supabase: SupabaseClient, ctx: ApiKeyContext, input: UpdateTemplatePackageInput) {
  requireScope(ctx, "template_packages:write");
  ...
  if (Object.keys(updates).length > 0) {
    const { error } = await supabase.from("template_packages").update(updates).eq("id", input.package_id);
  ...
  if (Array.isArray(input.template_ids)) {
    ...
    if (input.template_ids.length > 0) {
      ...
      const { error: itemErr } = await supabase.from("template_package_items").insert(itemRows);
      if (itemErr) throw new ScopeError(`Uppdaterade mallpaket men mallkopplingar misslyckades: ${itemErr.message}`, 500);
    }
  }
