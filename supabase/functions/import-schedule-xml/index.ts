...
function usernameFromName(displayName: string): string {
  ...
  const first = transliterate(parts[0] ?? "")
    .toLowerCase()
    .replace(/[^a-z0-9]/g, "");
  const last = transliterate(parts.slice(1).join(""))
    .toLowerCase()
    .replace(/[^a-z0-9]/g, "");
...
function parseXmlDay(
  ...
  if (shifts.length === 0) {
    let isFirst = true;
    for (const sEl of Array.from(dayEl.children).filter(
      (c: El) => c.nodeName === "Shifts",
    ) as El[]) {
...
function parseXml(xmlText: string): ParsedSchedule[] | null {
  ...
  const employeeEls = Array.from(dataRoot.children).filter(
    (c: El) => c.nodeName === "Employee",
  ) as El[];
  if (employeeEls.length > 0 && employeeEls.some((e: El) => e.querySelector("Week"))) {
    ...
    for (const empEl of employeeEls) {
      ...
      for (const weekEl of weekEls) {
        ...
        const days: XmlDay[] = (
          Array.from(weekEl.children).filter((c: El) => c.nodeName === "Day") as El[]
  ...
  const directWeekEls = Array.from(dataRoot.children).filter(
    (c: El) => c.nodeName === "Week",
  ) as El[];
  if (directWeekEls.length > 0) {
    ...
    for (const weekEl of directWeekEls) {
      ...
      const employees: ParsedEmployee[] = (
        Array.from(weekEl.children).filter((c: El) => c.nodeName === "Employee") as El[]
      ).map((empEl: El) => {
        const days: XmlDay[] = (Array.from(empEl.querySelectorAll("Day")) as El[]).map(
          (dayEl: El) =>
  ...
  const rootEmpEls = Array.from(dataRoot.children).filter(
    (c: El) => c.nodeName === "Employee",
  ) as El[];
  if (rootEmpEls.length > 0) {
    ...
    const employees: ParsedEmployee[] = rootEmpEls.map((empEl: El) => {
      const days: XmlDay[] = (Array.from(empEl.querySelectorAll("Day")) as El[]).map((dayEl: El) =>
        parseXmlDay(dayEl, "", (date) => {
          if (!weekStartDate) weekStartDate = date;
          if (!year && date.length >= 4) year = parseInt(date.slice(0, 4), 10);
        }),
...
Deno.serve(async (req: Request) => {
  ...
  if (!hasScope(ctx, "schedule:write"))
    return json({ error: "Nyckeln saknar scope 'schedule:write'." }, 403);
  ...
  try {
    ...
    if (!trimmed) {
      return json(
        {
          error:
            'Request-body är tomt. Skicka JSON med minst { "store_id", "imported_by_user_id", "xml" } (Content-Type: application/json).',
        },
        400,
      );
    }
    if (trimmed.startsWith("<?xml") || trimmed.startsWith("<SOE_") || trimmed.startsWith("<Time")) {
      return json(
        {
          error:
            'Det ser ut som att du skickade rå XML direkt i request-body. XML-innehållet måste skickas inuti ett JSON-objekt, t.ex.: { "store_id": "...", "imported_by_user_id": "...", "xml": "<xml-innehåll>" } eller base64-kodat i fältet "xml_base64". Sätt Content-Type: application/json.',
        },
        400,
      );
    }
  ...
  if (!imported_by_user_id)
    return json(
      {
        error:
          "imported_by_user_id saknas (måste vara ett giltigt app_users.id — t.ex. ditt eget admin-konto eller ett dedikerat automationskonto).",
      },
      400,
    );
  if (!body.xml && !body.xml_base64)
  ...
  const { data: store } = await supabase
    .from("stores")
    .select("id, forening_id, distrikt_id")
    .eq("id", store_id)
    .maybeSingle();
  if (!store) return json({ error: `Ingen butik hittades med store_id ${store_id}.` }, 404);

  const { data: importedByUser } = await supabase
    .from("app_users")
    .select("id")
    .eq("id", imported_by_user_id)
    .maybeSingle();
  if (!importedByUser)
    return json(
      { error: `Ingen användare hittades med imported_by_user_id ${imported_by_user_id}.` },
      404,
    );
  ...
  if (!parsed || parsed.length === 0) {
    return json(
      { error: "Kunde inte tolka XML-filen. Kontrollera att det är en SoftOne GO-schemaexport." },
      422,
    );
  }
  ...
  const mappings = new Map<string, string | null>(
    (mappingRows ?? []).map((m: { employee_nr: string; app_user_id: string | null }) => [
      m.employee_nr,
      m.app_user_id,
    ]),
  );
  ...
  const { data: allUsers } = await supabase
    .from("app_users")
    .select("id, display_name, role_manually_set");
  const allUsersList = (allUsers ?? []) as {
    id: string;
    display_name: string;
  ...
  async function linkUserToStoreHierarchy(userId: string) {
    if (store.forening_id) {
      await supabase
        ...
        .upsert(
          { user_id: userId, forening_id: store.forening_id, is_primary: false },
      ...
      if (!existing?.forening_id)
        await supabase
          .from("app_users")
          .update({ forening_id: store.forening_id })
          .eq("id", userId);
    }
    if (store.distrikt_id) {
      await supabase
        .from("user_distrikt")
        .upsert(
          { user_id: userId, distrikt_id: store.distrikt_id, is_primary: false },
      ...
      if (!existing?.distrikt_id)
        await supabase
          .from("app_users")
          .update({ distrikt_id: store.distrikt_id })
          .eq("id", userId);
    }
  }
  ...
  for (const emp of allEmployees) {
    ...
    if (savedMapping) {
      ...
      await supabase
        .from("user_stores")
        .upsert(
          { user_id: savedMapping, store_id, is_primary: false },
      ...
      if (emp.employeeGroup && !existingUser?.role_manually_set) {
        await supabase
          .from("app_users")
          .update({ role: groupToRole(emp.employeeGroup), employee_group: emp.employeeGroup })
          .eq("id", savedMapping);
      } else if (emp.employeeGroup) {
        await supabase
          .from("app_users")
          .update({ employee_group: emp.employeeGroup })
          .eq("id", savedMapping);
      }
      continue;
    }
    ...
    const byStoreAndName = allUsersList.find(
      (u) =>
    ...
    const globalNameMatch = allUsersList.find(
      (u) =>
        !usedUserIds.has(u.id) &&
        normalizeName(u.display_name) === normEmp &&
        !storeUserIds.has(u.id),
    ...
    if (!autoCreateUsers) {
      ...
      warnings.push(
        `${emp.employeeName} (${emp.employeeNr}) kunde inte matchas mot ett konto och auto_create_users=false — inget konto skapades. Passet importerades ändå.`,
      );
      continue;
    }
    ...
    if (createErr || !created) {
      warnings.push(
        `Kunde inte skapa konto för ${emp.employeeName}: ${createErr?.message ?? "okänt fel"}. Passet importerades ändå.`,
      );
      finalMappings.set(emp.employeeNr, null);
      continue;
    }
  ...
  for (const weekSchedule of parsed) {
    ...
    const resolvedWeekStart =
      weekSchedule.weekStartDate || getWeekStartDate(weekSchedule.weekNumber, weekSchedule.year);
    const { data: importData, error: importErr } = await supabase
      ...
      .insert({
        ...
        filename: `power-automate_vecka_${weekSchedule.weekNumber}_${weekSchedule.year}.xml`,
    ...
    if (importErr || !importData)
      return json(
        {
          error: `schedule_imports vecka ${weekSchedule.weekNumber}: ${importErr?.message ?? "import misslyckades"}`,
        },
        500,
      );
    ...
    for (const emp of weekSchedule.employees) {
      ...
      const isEmpBorrowed = warnings.some(
        (w) => w.startsWith(emp.employeeName) && w.includes("lånad personal"),
      );
      ...
      if (rows.length > 0) {
        ...
        if (shiftsErr)
          warnings.push(
            `Kunde inte spara pass för ${emp.employeeName} (${emp.employeeNr}): ${shiftsErr.message}`,
          );
  ...
  async function upsertGroup(name: string, memberIds: string[]) {
    ...
    let { data: grp } = await supabase
      ...
      .maybeSingle();
    if (!grp) {
      const { data: createdGrp } = await supabase
        .from("user_groups")
  ...
  const { data: allUsersFull } = await supabase
    .from("app_users")
    .select("id, role, hierarchy_level")
    .in(
      "id",
      Array.from(finalMappings.values()).filter((v): v is string => !!v),
    );
  const managerIds = new Set(
    (allUsersFull ?? [])
      .filter(
        (u: { role: string; hierarchy_level?: string | null }) =>
          u.role === "manager" || u.hierarchy_level === "chef",
      )
