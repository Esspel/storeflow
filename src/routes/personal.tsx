...
function hierarchyBadge(level: string | null | undefined) {
  ...
  if (key === "hk" || key === "forening" || key === "distrikt")
    return <Badge className="bg-info/15 text-info">{label}</Badge>;
...
function hierarchyLevelToRole(level: string): "admin" | "manager" | "employee" {
  ...
  if (level === "hk" || level === "forening" || level === "distrikt" || level === "chef")
    return "manager";
  return "employee";
}
...
function parseStoreCsv(text: string): {
  ...
  if (lines.length < 2)
    return { rows: [], headers: [], error: "Filen verkar tom eller saknar rubrikrad." };
  ...
  if (!hasName) {
    return {
      rows: [],
      headers,
      error: `Importen avbröts: Saknad obligatorisk kolumn "Butik / Enhet". Kontrollera att filen har rätt format.`,
    };
  }
...
function AccountsPage() {
  ...
  const [userSortField, setUserSortField] = useState<"display_name" | "role" | "created_at">(
    "display_name",
  );
  const [userSortDir, setUserSortDir] = useState<SortDir>("asc");
  const [storeSearch, setStoreSearch] = useState("");
  const [storeSortField, setStoreSortField] = useState<
    "name" | "bolag" | "distrikt_namn" | "butiks_nr" | "koncept"
  >("name");
  ...
  const [editUser, setEditUser] = useState<
    (UserWithStores & { forening_id?: string | null; distrikt_id?: string | null }) | null
  >(null);
  ...
  const [newUser, setNewUser] = useState({
    ...
    pin: "",
    barcode: "",
    forening_id: "" as string,
    distrikt_id: "" as string,
  });
  ...
  const [userCsvResult, setUserCsvResult] = useState<{
    success: number;
    skipped: number;
    errors: string[];
  } | null>(null);
  ...
  const [groups, setGroups] = useState<
    (UserGroup & { members?: (UserGroupMember & { user?: AppUser })[] })[]
  >([]);
  ...
  const newUserFilteredStores = useMemo(() => {
    ...
    return stores
      .filter((s) => {
        ...
        if (!newUser.distrikt_id && newUser.forening_id && s.forening_id !== newUser.forening_id)
          return false;
        return (
          !q ||
          s.name.toLowerCase().includes(q) ||
          (s.butiks_nr && String(s.butiks_nr).includes(q)) ||
  ...
  const editUserFilteredStores = useMemo(() => {
    ...
    return stores
      .filter((s) => {
        ...
        if (!editUser.distrikt_id && editUser.forening_id && s.forening_id !== editUser.forening_id)
          return false;
        return (
          !q ||
          s.name.toLowerCase().includes(q) ||
  ...
  async function load() {
    ...
    if (!isAdmin) {
      ...
      usersWithStores = usersWithStores.filter(
        (u) =>
...
`;
  ...
  function exportUsersCsv() {
    const rows = [
      ...
      ...users.map((u) => [
        ...
        u.barcode_id ?? "",
        stores
          .filter((s) => u.assignedStoreIds.includes(s.id))
          .map((s) => s.butiks_nr ?? "")
          .filter(Boolean)
          .join(","),
      ]),
  ...
  async function importUsersCsv(file: File) {
    ...
    for (const line of dataLines) {
      ...
      if (pw.length < MIN_PW_LENGTH) {
        errors.push(`${username}: Lösenordet är för kort`);
        skipped++;
        continue;
      }
      ...
      const { data: existing } = await supabase
        ...
        .maybeSingle();
      if (existing) {
        errors.push(`${username}: Användarnamnet finns redan`);
        skipped++;
        continue;
      }
      ...
      if (created?.id) {
        ...
        if (butiksnummer?.trim()) {
          const nrs = butiksnummer
            .split(",")
            .map((s) => s.trim())
            .filter(Boolean);
          const matchedStores = stores.filter(
            (s) => s.butiks_nr && nrs.includes(String(s.butiks_nr)),
          );
          if (matchedStores.length > 0)
            await syncUserStores(
              created.id,
              matchedStores.map((s) => s.id),
            );
        }
  ...
  async function syncUserStores(userId: string, storeIds: string[]) {
    const allowedIds = isAdmin
      ? storeIds
      : storeIds.filter((sid) => manageableStoreIds?.includes(sid));
  ...
  async function syncUserHierarchyFromStores(userId: string, storeIds: string[]) {
    ...
    const foreningIds = [
      ...new Set(linkedStores.map((s) => s.forening_id).filter(Boolean) as string[]),
    ];
    const distriktIds = [
      ...new Set(linkedStores.map((s) => s.distrikt_id).filter(Boolean) as string[]),
    ];
  ...
  const createUser = async () => {
    ...
    if (newUser.password.length < MIN_PW_LENGTH) {
    ...
    if (!isUnrestricted) {
      ...
      if (targetRank > myRank) {
        setError("Du kan inte skapa användare med högre hierarkinivå än dig själv.");
        return;
      }
    }
    setSaving(true);
    ...
    const { data: hash } = await supabase.rpc("hash_password", {
      plain_password: newUser.password,
    });
    ...
    const safeStoreIds = isUnrestricted
      ? newUser.storeIds
    ...
    if (newUser.barcode.trim()) {
      const { data: existingBarcode } = await supabase
        .from("app_users")
        .select("id")
        .eq("barcode_id", newUser.barcode.trim())
        .maybeSingle();
      if (existingBarcode) {
        setError("Streckkoden är redan registrerad på en annan användare.");
        setSaving(false);
        return;
      }
    }
    ...
    if (created?.id) {
      ...
      logAudit(currentUser?.id ?? null, "user.create", "app_users", created.id, {
  ...
  const updateUser = async () => {
    if (!editUser) return;
    if (resetPw && resetPw.length < MIN_PW_LENGTH) {
      setError(`Nytt lösenord måste vara minst ${MIN_PW_LENGTH} tecken.`);
      return;
    }
    if (editPin && editPin.length > 0 && editPin.length < 4) {
    ...
    if (!isUnrestricted) {
      const myStoreIds = currentUserStores.map((s) => s.id);
      const sharesStore =
        editUser.assignedStoreIds.some((sid) => myStoreIds.includes(sid)) ||
        editUser.id === currentUser?.id;
      if (!sharesStore) {
        setError("Du har inte behörighet att redigera denna användare.");
        return;
      }
      ...
      if (targetOriginal && hierarchyRank(targetOriginal.hierarchy_level) > myRank) {
        setError("Du kan inte redigera användare med högre hierarkinivå än dig själv.");
        return;
      }
      // Cannot escalate target ABOVE own level
      if (hierarchyRank(editUser.hierarchy_level) > myRank) {
        setError("Du kan inte sätta en hierarkinivå som är högre än din egen.");
        return;
      }
    }
    setSaving(true);
    if (editBarcode.trim()) {
      const { data: existingBarcode } = await supabase
        .from("app_users")
        .select("id")
        .eq("barcode_id", editBarcode.trim())
        .neq("id", editUser.id)
        .maybeSingle();
      if (existingBarcode) {
        setError("Streckkoden är redan registrerad på en annan användare.");
  ...
  const confirmDeleteUser = async () => {
    ...
    logAudit(currentUser?.id ?? null, "user.delete", "app_users", deleteUser.id, {
  ...
  const createStore = async () => {
    ...
    const { data: created } = await supabase
      ...
      .select("id")
      .maybeSingle();
    logAudit(currentUser?.id ?? null, "store.create", "stores", created?.id ?? null, {
      name: newStore.name,
    });
  ...
  const updateStore = async () => {
    ...
    await supabase
      ...
      .eq("id", editStore.id);
    logAudit(currentUser?.id ?? null, "store.edit", "stores", editStore.id, {
      name: editStore.name,
    });
    const { data } = await supabase.from("stores").select("*").order("name");
    setStores((data ?? []) as Store[]);
  ...
  const confirmDeleteStore = async () => {
    ...
    logAudit(currentUser?.id ?? null, "store.delete", "stores", deleteStore.id, {
  ...
  const handleCsvFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const result: CsvImportResult & { updated: number } = {
        success: 0,
        updated: 0,
        skipped: 0,
        errors: [],
        ...
        try {
          if (butikNr) {
            ...
            const { error: insertErr } = await supabase
              .from("stores")
              .insert({ ...payload, butiks_nr: butikNr });
            if (insertErr) {
  ...
  async function createGroup() {
    ...
    if (!newGroup.name.trim()) {
      setError("Gruppnamn obligatoriskt.");
      return;
    }
    if (!isAdmin && newGroup.store_id && !manageableStoreIds?.includes(newGroup.store_id)) {
      setError("Du kan bara skapa grupper i dina egna butiker.");
      return;
    }
    if (!isAdmin && !newGroup.store_id) {
      setError("Välj vilken butik gruppen tillhör.");
      return;
    }
    setSaving(true);
    const { data: created } = await supabase
    ...
    if (created?.id && newGroup.memberIds.length > 0) {
      await supabase
        ...
        .insert(newGroup.memberIds.map((uid) => ({ group_id: created.id, user_id: uid })));
    }
    logAudit(currentUser?.id ?? null, "group.create", "user_groups", created?.id ?? null, {
      name: newGroup.name,
    });
    await loadGroups();
    setSaving(false);
  ...
  async function saveEditGroup() {
    ...
    if (!isAdmin && editGroup.store_id && !manageableStoreIds?.includes(editGroup.store_id)) {
      setError("Du har inte behörighet att redigera denna grupp.");
      return;
    }
    setSaving(true);
    ...
    if (editGroup.memberIds.length > 0) {
      await supabase
        .from("user_group_members")
        .insert(editGroup.memberIds.map((uid) => ({ group_id: editGroup.id, user_id: uid })));
    }
  ...
  async function confirmDeleteGroup() {
    ...
    if (!isAdmin && deleteGroup.store_id && !manageableStoreIds?.includes(deleteGroup.store_id))
      return;
    await supabase.from("user_groups").delete().eq("id", deleteGroup.id);
    logAudit(currentUser?.id ?? null, "group.delete", "user_groups", deleteGroup.id, {
  ...
  const filteredStores = useMemo(() => {
    ...
    list = list.sort((a, b) => {
      ...
      if (storeSortField === "name") {
        av = a.name;
        bv = b.name;
      } else if (storeSortField === "bolag") {
        av = a.bolag ?? "";
        bv = b.bolag ?? "";
      } else if (storeSortField === "distrikt_namn") {
        av = a.distrikt_namn ?? "";
        bv = b.distrikt_namn ?? "";
      } else if (storeSortField === "butiks_nr") {
        av = a.butiks_nr ?? "";
        bv = b.butiks_nr ?? "";
      } else if (storeSortField === "koncept") {
        av = a.koncept ?? "";
        bv = b.koncept ?? "";
      }
      const cmp = av.localeCompare(bv, "sv");
  ...
  return (
    <div className="mx-auto max-w-[1400px] px-4 py-6 md:px-8 md:py-10">
      <PageHeader
        ...
        description={
          isAdmin
            ? "Hantera användarkonton, butiker och grupper."
            : "Hantera personal och grupper i dina butiker."
        }
      />
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {isManager && (
                <Button
                  ...
                  onClick={() => userCsvInputRef.current?.click()}
                >
                  <Upload className="mr-1.5 h-3.5 w-3.5" />{" "}
                  {userCsvImporting ? "Importerar..." : "Importera CSV"}
                </Button>
              )}
              <input
                ref={userCsvInputRef}
                    ...
                    {filteredUsers.map((u) => (
                      <tr key={u.id} className="hover:bg-muted/30">
                        ...
                        <td className="px-4 py-3 text-center">
                          {hierarchyBadge(u.hierarchy_level)}
                        </td>
                        <td className="px-4 py-3 text-center">
                          <Switch
                            checked={u.is_active}
                            onCheckedChange={() =>
                              u.id !== currentUser?.id && toggleUserActive(u.id, u.is_active)
                            }
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-1">
                      <Button
                        ...
                        className="rounded-full text-xs"
                        onClick={() =>
                          setEditGroup({ ...g, memberIds: g.members?.map((m) => m.user_id) ?? [] })
                        }
                      >
                        Redigera
                      </Button>
                ...
                <div className="flex flex-wrap gap-2">
                  <Button
                    ...
                    onClick={async () => {
                      ...
                      exportTextAsCSV(
                        csv,
                        `uppgifter-${activeStore.name}-${new Date().toISOString().slice(0, 10)}.csv`,
                      );
                    }}
                  >
                  ...
                  <Button
                    ...
                    onClick={async () => {
                      ...
                      exportTextAsCSV(
                        csv,
                        `avvikelser-${activeStore.name}-${new Date().toISOString().slice(0, 10)}.csv`,
                      );
                    }}
                  >
                  ...
                  <Button
                    ...
                    onClick={async () => {
                      ...
                      const { data } = await supabase
                        ...
                        .select(
                          "id, product_name, article_number, priority, status, staff_comment, created_at",
                        )
                        .eq("store_id", activeStore.id)
                        .order("created_at", { ascending: false })
                        .limit(2000);
                      ...
                      exportTextAsCSV(
                        csv,
                        `kundonskemal-${activeStore.name}-${new Date().toISOString().slice(0, 10)}.csv`,
                      );
                    }}
                  >
          ...
          <TabsContent value="stores" className="mt-6">
            ...
            {csvPreviewError && (
              <div className="mb-4 flex items-start gap-3 rounded-xl border border-destructive/30 bg-destructive/5 px-4 py-3">
                ...
                <button
                  onClick={() => setCsvPreviewError(null)}
              ...
              <div className="mb-4 rounded-xl border border-border/60 bg-card p-4">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <p className="text-sm font-medium">
                    Import klar — {csvResult.success} nya, {csvResult.updated} uppdaterade
                    ...
                    <div className="mt-3 space-y-1 text-xs text-muted-foreground">
                      {store.butiks_nr && (
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="font-mono">#{store.butiks_nr}</span>
                        </div>
                      )}
                      {store.bolag && (
                        <div className="flex items-center gap-1.5">
                          <Building2 className="h-3.5 w-3.5 shrink-0" />
                          <span>{store.bolag}</span>
                        </div>
                      )}
                      {store.distrikt_namn && (
                        <div className="flex items-center gap-1.5">
                      ...
                      {(store.gatuadress || store.postadress) && (
                        <div className="flex items-center gap-1.5">
                          ...
                          <span>
                            {[store.gatuadress, store.postnr, store.postadress]
                              .filter(Boolean)
                              .join(", ")}
                          </span>
                        </div>
                      ...
                      {store.email_sm_chef && (
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="truncate">{store.email_sm_chef}</span>
                        </div>
                      )}
                      {store.telefon_butik && (
                        <div className="flex items-center gap-1.5">
                          <Phone className="h-3.5 w-3.5 shrink-0" />
                          <span>{store.telefon_butik}</span>
                        </div>
                      )}
                      {store.butikschef && (
                        <div className="flex items-center gap-1.5">
                          <UserCog className="h-3.5 w-3.5 shrink-0" />
                          <span>{store.butikschef}</span>
                        </div>
                      )}
                      {store.site_id && (
                        <div className="flex items-center gap-1.5">
                          <Hash className="h-3.5 w-3.5 shrink-0" />
                          <span className="font-mono text-[10px]">Site-ID {store.site_id}</span>
                        </div>
                      )}
                    </div>
                  </div>
        ...
        <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="space-y-1.5">
              ...
              <Select
                value={newUser.hierarchy_level}
                onValueChange={(v) =>
                  setNewUser((p) => ({
                    ...p,
                    hierarchy_level: v,
                    role: hierarchyLevelToRole(v),
                    forening_id: "",
                    distrikt_id: "",
                  }))
                }
              >
                ...
                <SelectContent>
                  {Object.entries(HIERARCHY_LABELS)
                    .filter(([val]) => {
                      const myRank = effectiveRank(currentUser);
            ...
            {newUser.hierarchy_level !== "hk" && newUser.hierarchy_level !== "admin" && (
              <div className="space-y-1.5">
                ...
                <Select
                  value={newUser.forening_id || "__none__"}
                  onValueChange={(v) =>
                    setNewUser((p) => ({
                      ...p,
                      forening_id: v === "__none__" ? "" : v,
                      distrikt_id: "",
                      storeIds: [],
                    }))
                  }
            ...
            {newUser.hierarchy_level !== "hk" &&
              newUser.hierarchy_level !== "admin" &&
                ...
                <div className="space-y-1.5">
                  ...
                  <Select
                    ...
                    onValueChange={(v) => {
                      ...
                      setNewUser((p) => ({
                        ...p,
                        distrikt_id: v === "__none__" ? "" : v,
                        forening_id: d?.forening_id || p.forening_id,
                        storeIds: [],
                      }));
                    }}
                  >
            ...
            <div className="space-y-1.5">
              ...
              <Input
                placeholder="t.ex. Butik Timlön"
                value={newUser.employee_group}
            ...
            <div className="space-y-1.5">
              ...
              <div className="max-h-40 overflow-y-auto rounded-lg border border-border/60 p-2 space-y-1">
                {newUserFilteredStores.length > 0 ? (
                  newUserFilteredStores.map((s) => (
                    <label
                      key={s.id}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  value={newUser.pin}
                  onChange={(e) =>
                    setNewUser((p) => ({ ...p, pin: e.target.value.replace(/\D/g, "") }))
                  }
                  autoComplete="off"
                />
              </div>
              <div className="space-y-1.5">
                ...
                <div className="flex gap-2">
                  ...
                  />
                  <BarcodeScanButton
                    onScan={(code) => setNewUser((p) => ({ ...p, barcode: code }))}
                  />
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4 py-2">
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setEditUser((u) => (u ? { ...u, display_name: e.target.value } : null))
                  }
                />
              </div>
              <div className="space-y-1.5">
                <Label>Hierarkinivå</Label>
                <Select
                  value={editUser.hierarchy_level ?? "anvandare"}
                  onValueChange={(v) =>
                    setEditUser((u) =>
                      u
                        ? {
                            ...u,
                            hierarchy_level: v as AppUser["hierarchy_level"],
                            role: hierarchyLevelToRole(v),
                            forening_id: null,
                            distrikt_id: null,
                          }
              ...
              {editUser.hierarchy_level !== "hk" && editUser.hierarchy_level !== "admin" && (
                <div className="space-y-1.5">
                  ...
                  <Select
                    value={editUser.forening_id ?? "__none__"}
                    onValueChange={(v) =>
                      setEditUser((u) =>
                        u
                          ? {
                              ...u,
                              forening_id: v === "__none__" ? null : v,
                              distrikt_id: null,
                              assignedStoreIds: [],
                            }
                          : null,
              ...
              {editUser.hierarchy_level !== "hk" &&
                editUser.hierarchy_level !== "admin" &&
                      ...
                      onValueChange={(v) => {
                        ...
                        setEditUser((u) =>
                          u
                            ? {
                                ...u,
                                distrikt_id: v === "__none__" ? null : v,
                                forening_id: d?.forening_id ?? u.forening_id,
                                assignedStoreIds: [],
                              }
                            : null,
                        );
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  placeholder="t.ex. Butik Timlön"
                  value={editUser.employee_group ?? ""}
                  onChange={(e) =>
                    setEditUser((u) => (u ? { ...u, employee_group: e.target.value } : null))
                  }
                />
              </div>
              <div className="space-y-1.5">
                ...
                <div className="max-h-40 overflow-y-auto rounded-lg border border-border/60 p-2 space-y-1">
                  {editUserFilteredStores.length > 0 ? (
                    editUserFilteredStores.map((s) => (
                      <label
                        key={s.id}
                        className="flex cursor-pointer items-center gap-2 rounded px-2 py-1 hover:bg-muted/50"
                      >
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  placeholder="t.ex. Butik 123"
                  value={newStore.bolag}
      ...
      >
        {editStore && (
          <DialogContent className="max-h-[90vh] max-w-lg overflow-y-auto">
            <DialogHeader>
              <DialogTitle>Redigera butik</DialogTitle>
            </DialogHeader>
            <div className="space-y-4 py-2">
              <div className="space-y-1.5">
                <Label>Butiksnamn *</Label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  <Label>Butiksnummer</Label>
                  <Input
                    value={editStore.butiks_nr ?? ""}
                    onChange={(e) =>
                      setEditStore((s) => (s ? { ...s, butiks_nr: e.target.value } : null))
                    }
                  />
                </div>
                <div className="space-y-1.5">
                  <Label>Bolag (Förening)</Label>
                  <Input
                    value={editStore.bolag ?? ""}
                    onChange={(e) =>
                      setEditStore((s) => (s ? { ...s, bolag: e.target.value } : null))
                    }
                  />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  <Label>Koncept</Label>
                  <Input
                    value={editStore.koncept ?? ""}
                    onChange={(e) =>
                      setEditStore((s) => (s ? { ...s, koncept: e.target.value } : null))
                    }
                  />
                </div>
                <div className="space-y-1.5">
                  ...
                  <Input
                    value={editStore.distrikt_namn ?? ""}
                    onChange={(e) =>
                      setEditStore((s) => (s ? { ...s, distrikt_namn: e.target.value } : null))
                    }
                  />
                </div>
              </div>
              <div className="space-y-1.5">
                ...
                <Input
                  value={editStore.gatuadress ?? ""}
                  onChange={(e) =>
                    setEditStore((s) => (s ? { ...s, gatuadress: e.target.value } : null))
                  }
                />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <Input
                    value={editStore.postnr ?? ""}
                    onChange={(e) =>
                      setEditStore((s) => (s ? { ...s, postnr: e.target.value } : null))
                    }
                  />
                </div>
                <div className="space-y-1.5">
                  <Label>Stad</Label>
                  <Input
                    value={editStore.postadress ?? ""}
                    onChange={(e) =>
                      setEditStore((s) => (s ? { ...s, postadress: e.target.value } : null))
                    }
                  />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <Input
                    value={editStore.butikschef ?? ""}
                    onChange={(e) =>
                      setEditStore((s) => (s ? { ...s, butikschef: e.target.value } : null))
                    }
                  />
                </div>
                <div className="space-y-1.5">
                  ...
                  <Input
                    type="email"
                    value={editStore.email_sm_chef ?? ""}
                    onChange={(e) =>
                      setEditStore((s) => (s ? { ...s, email_sm_chef: e.target.value } : null))
                    }
                  />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <Input
                    value={editStore.telefon_butik ?? ""}
                    onChange={(e) =>
                      setEditStore((s) => (s ? { ...s, telefon_butik: e.target.value } : null))
                    }
                  />
                </div>
                <div className="space-y-1.5">
                  <Label>Kommun</Label>
                  <Input
                    value={editStore.kommun ?? ""}
                    onChange={(e) =>
                      setEditStore((s) => (s ? { ...s, kommun: e.target.value } : null))
                    }
                  />
                </div>
              </div>
              <div className="space-y-1.5">
                <Label>SAP Site-ID (produktkatalog)</Label>
                <Input
                  value={editStore.site_id ?? ""}
                  onChange={(e) =>
                    setEditStore((s) =>
                      s ? { ...s, site_id: e.target.value, sap_site_id: e.target.value } : null,
                    )
                  }
                />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <Input
                    value={editStore.bc_telefon ?? ""}
                    onChange={(e) =>
                      setEditStore((s) => (s ? { ...s, bc_telefon: e.target.value } : null))
                    }
                  />
                </div>
                <div className="space-y-1.5">
                  <Label>Mobil</Label>
                  <Input
                    value={editStore.mobil ?? ""}
                    onChange={(e) =>
                      setEditStore((s) => (s ? { ...s, mobil: e.target.value } : null))
                    }
                  />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <Input
                    value={editStore.marknadsomrade ?? ""}
                    onChange={(e) =>
                      setEditStore((s) => (s ? { ...s, marknadsomrade: e.target.value } : null))
                    }
                  />
                </div>
                <div className="space-y-1.5">
                  ...
                  <Input
                    value={editStore.distriktschef ?? ""}
                    onChange={(e) =>
                      setEditStore((s) => (s ? { ...s, distriktschef: e.target.value } : null))
                    }
                  />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <Input
                    value={editStore.direktor_forsaljning ?? ""}
                    onChange={(e) =>
                      setEditStore((s) =>
                        s ? { ...s, direktor_forsaljning: e.target.value } : null,
                      )
                    }
                  />
                </div>
                <div className="space-y-1.5">
                  ...
                  <Input
                    value={editStore.forsaljningschef ?? ""}
                    onChange={(e) =>
                      setEditStore((s) => (s ? { ...s, forsaljningschef: e.target.value } : null))
                    }
                  />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  <Label>Företag</Label>
                  <Input
                    value={editStore.foretag ?? ""}
                    onChange={(e) =>
                      setEditStore((s) => (s ? { ...s, foretag: e.target.value } : null))
                    }
                  />
                </div>
                <div className="space-y-1.5">
                  <Label>Enhet</Label>
                  <Input
                    value={editStore.enhet ?? ""}
                    onChange={(e) =>
                      setEditStore((s) => (s ? { ...s, enhet: e.target.value } : null))
                    }
                  />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <Input
                    value={editStore.organisationsnummer ?? ""}
                    onChange={(e) =>
                      setEditStore((s) =>
                        s ? { ...s, organisationsnummer: e.target.value } : null,
                      )
                    }
                  />
                </div>
                <div className="space-y-1.5">
                  ...
                  <Input
                    value={editStore.k_stalle ?? ""}
                    onChange={(e) =>
                      setEditStore((s) => (s ? { ...s, k_stalle: e.target.value } : null))
                    }
                  />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <Input
                    value={editStore.gamla_butiksnummer ?? ""}
                    onChange={(e) =>
                      setEditStore((s) => (s ? { ...s, gamla_butiksnummer: e.target.value } : null))
                    }
                  />
                </div>
                <div className="space-y-1.5">
                  ...
                  <Input
                    value={editStore.saljplan ?? ""}
                    onChange={(e) =>
                      setEditStore((s) => (s ? { ...s, saljplan: e.target.value } : null))
                    }
                  />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <Input
                    value={editStore.hr_generalist ?? ""}
                    onChange={(e) =>
                      setEditStore((s) => (s ? { ...s, hr_generalist: e.target.value } : null))
                    }
                  />
                </div>
                <div className="space-y-1.5">
                  ...
                  <Input
                    value={editStore.bemanningsspecialist ?? ""}
                    onChange={(e) =>
                      setEditStore((s) =>
                        s ? { ...s, bemanningsspecialist: e.target.value } : null,
                      )
                    }
                  />
                </div>
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  value={editStore.sak_kval_samordnare ?? ""}
                  onChange={(e) =>
                    setEditStore((s) => (s ? { ...s, sak_kval_samordnare: e.target.value } : null))
                  }
                />
              </div>
              <div className="space-y-1.5">
                ...
                <Input
                  value={editStore.kommentar ?? ""}
                  onChange={(e) =>
                    setEditStore((s) => (s ? { ...s, kommentar: e.target.value } : null))
                  }
                />
              </div>
              {error && (
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              <span className="mt-2 block text-xs">
                Relaterade uppgifter, avvikelser och användarkopplingar kan påverkas. Åtgärden kan
                inte ångras.
              </span>
            </AlertDialogDescription>
          </AlertDialogHeader>
      ...
      >
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Ny grupp</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-2">
            <div className="space-y-1.5">
              <Label>Gruppnamn *</Label>
              <Input
                placeholder="T.ex. Morgonpersonal"
                value={newGroup.name}
            ...
            <div className="space-y-1.5">
              ...
              <Select
                value={newGroup.store_id || "__none"}
                onValueChange={(v) =>
                  setNewGroup((p) => ({ ...p, store_id: v === "__none" ? "" : v }))
                }
              >
                <SelectTrigger>
                  <SelectValue placeholder={isAdmin ? "Alla butiker" : "Välj butik"} />
                </SelectTrigger>
                <SelectContent>
              ...
              <div className="max-h-40 overflow-y-auto rounded-lg border border-border/60 p-2 space-y-1">
                {users
                  ...
                  .filter(
                    (u) =>
                      !groupMemberSearch ||
                      u.display_name.toLowerCase().includes(groupMemberSearch.toLowerCase()),
                  )
                  .map((u) => (
                    <label
                      key={u.id}
                    ...
                    >
                      <Checkbox
                        ...
                        onCheckedChange={() =>
                          setNewGroup((p) => ({
                            ...p,
                            memberIds: p.memberIds.includes(u.id)
                          ...
                          onCheckedChange={() =>
                            setEditGroup((g) =>
                              g
                                ? {
                                    ...g,
                                    memberIds: g.memberIds.includes(u.id)
                                      ? g.memberIds.filter((id) => id !== u.id)
                                      : [...g.memberIds, u.id],
                                  }
                                : null,
                            )
