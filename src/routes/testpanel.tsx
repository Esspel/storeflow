...
function TestPanel() {
  ...
  async function loadStats() {
    ...
    setStats({
      ...
      scheduleImports: si.count ?? 0,
      scheduleShifts: ss.count ?? 0,
      deliveryPlans: dp.count ?? 0,
      auditEntries: al.count ?? 0,
      taskImages: ti.count ?? 0,
      incidentImages: ii.count ?? 0,
  ...
  function applyTimeSimulation() {
    ...
    addResult(
      true,
      `Tid framflyttad +${amount} ${timeUnit}. Simulerad tid: ${getSimulatedDate().toLocaleString("sv-SE")}`,
    );
  }
  ...
  async function resetTime() {
    if (
      !window.confirm(
        "Återställ simulerad tid och radera simulerade uppgifter? Detta kan inte ångras.",
      )
    )
      return;
  ...
  async function clearMyNotifications() {
    if (!window.confirm("Rensa alla dina notiser? Detta kan inte ångras.")) return;
    setRunning(true);
  ...
  async function clearAllNotificationsAllUsers() {
    if (!window.confirm("Radera ALLA notiser för alla användare? Detta kan inte ångras.")) return;
    setRunning(true);
  ...
  async function clearReadNotificationsAllUsers() {
    if (!window.confirm("Radera alla lästa notiser för alla användare? Detta kan inte ångras."))
      return;
    setRunning(true);
  ...
  async function checkPushStatus() {
    ...
    addResult(
      true,
      `VAPID-nyckel (klient): ${vapidConfigured ? "konfigurerad" : "SAKNAS — krävs för att aktivera push"}`,
    );
    addResult(
      true,
      `Stöd: Service Worker ${swSupported ? "JA" : "nej"}, PushManager ${pushSupported ? "JA" : "nej"}, Notification ${notifSupported ? "JA" : "nej"}`,
    );
    ...
    if (swSupported && pushSupported) {
      try {
        ...
        if (sub) {
          addResult(
            true,
            `Prenumererad i denna webbläsare: JA — endpoint: ${sub.endpoint.slice(0, 70)}…`,
          );
        } else {
          addResult(
            false,
            "Prenumererad i denna webbläsare: NEJ — aktivera notiser i Inställningar.",
          );
        }
      } catch (e) {
  ...
  async function sendTestOsPush() {
    ...
    try {
      ...
      if (!res.ok) {
        addResult(
          false,
          `OS-push misslyckades (HTTP ${res.status}): ${json?.error ?? "okänt fel"}`,
        );
      } else {
  ...
  async function sendStorePush() {
    if (!activeStore) {
      addResult(false, "Välj en aktiv butik först.");
      return;
    }
    setRunning(true);
    const url = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/send-push`;
    try {
      ...
      if (!res.ok) {
        addResult(
          false,
          `Store-push misslyckades (HTTP ${res.status}): ${json?.error ?? "okänt fel"}`,
        );
      } else {
        addResult(
          (json?.sent ?? 0) > 0,
  ...
  async function unregisterPushSubscription() {
    ...
    try {
      ...
      if (error)
        addResult(
          false,
          `Webbläsare avprenumererad men DB-radering misslyckades: ${error.message}`,
        );
  ...
  async function sendNotificationToStore() {
    ...
    if (error) {
      addResult(false, `Kunde inte hämta butiksanvändare: ${error.message}`);
      setRunning(false);
      return;
    }
  ...
  async function listMyNotifications() {
    ...
    const { data, error } = await supabase
      ...
      .limit(5);
    if (error) {
      addResult(false, `Hämtning misslyckades: ${error.message}`);
      setRunning(false);
      return;
    }
    if ((data ?? []).length === 0) {
  ...
  async function bulkCreateTasks() {
    ...
    if (error) addResult(false, `Bulk-uppgifter misslyckades: ${error.message}`);
      ...
      addResult(
        true,
        `6 bulk-uppgifter skapade (${(data ?? []).map((d) => d.id.slice(0, 6)).join(", ")}).`,
      );
    await loadStats();
    setRunning(false);
  }
  ...
  async function createOverdueTask() {
    ...
    if (error) addResult(false, `Misslyckades: ${error.message}`);
      ...
      addResult(
        true,
        `Försenad uppgift skapad med due_date ${dueDate} (id: ${data?.id?.slice(0, 8)}).`,
      );
    await loadStats();
    setRunning(false);
  }
  ...
  async function createTaskWithQuestions() {
    ...
    const { data: task, error } = await supabase
      ...
      .maybeSingle();
    if (error || !task?.id) {
      addResult(false, `Misslyckades: ${error?.message}`);
      setRunning(false);
      return;
    }
    await supabase.from("task_questions").insert([
      {
        task_id: task.id,
        label: "Är temperaturen kontrollerad?",
  ...
  async function createRecurringTask() {
    ...
    if (error) addResult(false, `Misslyckades: ${error.message}`);
      ...
      addResult(
        true,
        `Återkommande uppgift skapad med due_date ${simDate} (id: ${data?.id?.slice(0, 8)}).`,
      );
    await loadStats();
    setRunning(false);
  }
  ...
  async function deleteAllTestTasks() {
    if (!window.confirm("Radera alla [TEST]-uppgifter inklusive bilder? Detta kan inte ångras."))
      return;
    setRunning(true);
  ...
  async function checkOrphanedTaskImages() {
    ...
    if (!images || images.length === 0) {
      addResult(true, "Inga task_images-rader att kontrollera.");
      setRunning(false);
      return;
    }
    ...
    if (orphaned.length === 0) {
      ...
      addResult(
        false,
        `${orphaned.length} herrelösa task-bildrader hittade! IDs: ${orphaned.map((o) => o.id.slice(0, 8)).join(", ")}`,
      );
    }
    setRunning(false);
  }
  ...
  async function purgeOrphanedTaskImages() {
    if (!window.confirm("Rensa herrelösa task-bilder (DB + Storage)? Detta kan inte ångras."))
      return;
    setRunning(true);
    const { data: images } = await supabase.from("task_images").select("id, task_id, storage_path");
    if (!images || images.length === 0) {
      addResult(true, "Inga bilder att rensa.");
      setRunning(false);
      return;
    }
    const taskIds = [...new Set(images.map((i) => i.task_id))];
    ...
    const orphaned = images.filter((i) => !existingIds.has(i.task_id));
    if (orphaned.length === 0) {
      addResult(true, "Inga herrelösa bilder att rensa.");
      setRunning(false);
      return;
    }
  ...
  async function checkOrphanedIncidentImages() {
    ...
    const { data: images } = await supabase
      ...
      .select("id, incident_id, storage_path");
    if (!images || images.length === 0) {
      addResult(true, "Inga incident_images-rader att kontrollera.");
      setRunning(false);
      return;
    }
    ...
    if (orphaned.length === 0) {
      ...
      addResult(
        false,
        `${orphaned.length} herrelösa incident-bildrader! IDs: ${orphaned.map((o) => o.id.slice(0, 8)).join(", ")}`,
      );
    }
    setRunning(false);
  }
  ...
  async function purgeOrphanedIncidentImages() {
    if (!window.confirm("Rensa herrelösa incident-bilder (DB + Storage)? Detta kan inte ångras."))
      return;
    setRunning(true);
    const { data: images } = await supabase
      .from("incident_images")
    ...
    const orphaned = images.filter((i) => !existingIds.has(i.incident_id));
    if (orphaned.length === 0) {
      addResult(true, "Inga herrelösa incident-bilder att rensa.");
      setRunning(false);
      return;
    }
    deleteStorageFiles(orphaned.map((o) => o.storage_path));
  ...
  async function testImageCompression() {
    ...
    canvas.toBlob(async (blob) => {
      if (!blob) {
        addResult(false, "Kunde inte skapa testbild.");
        setRunning(false);
        return;
      }
      ...
      addResult(
        true,
        `Bildkomprimering OK: ${(original.size / 1024).toFixed(0)} KB → ${(compressed.size / 1024).toFixed(0)} KB (−${ratio}%, format: ${compressed.type})`,
      );
      setRunning(false);
    }, "image/png");
  }
  ...
  async function deleteAllTestIncidents() {
    if (!window.confirm("Radera alla [TEST]-avvikelser? Detta kan inte ångras.")) return;
    setRunning(true);
  ...
  async function deleteAllIncidentsForStore() {
    ...
    if (
      !window.confirm(
        `Radera ALLA avvikelser för ${activeStore.name} inklusive bilder? Detta kan inte ångras.`,
      )
    )
      return;
    setRunning(true);
    ...
    if (error) addResult(false, `Misslyckades: ${error.message}`);
      ...
      addResult(
        true,
        `${count ?? incIds.length} avvikelser raderade för ${activeStore.name} (inkl. bilder).`,
      );
    await loadStats();
    setRunning(false);
  }
  ...
  async function testTemplateUsage() {
    ...
    const { data, error } = await supabase
      ...
      .select(
        "id, title, items:checklist_template_items(*), questions:checklist_template_questions(*)",
      )
      .limit(3);
    if (error) {
      ...
      addResult(
        true,
        `Hämtade ${(data ?? []).length} mallar. ${(data ?? []).map((t) => `"${t.title}" (${(t.items ?? []).length} steg, ${(t.questions ?? []).length} frågor)`).join("; ") || "Inga mallar."}`,
      );
    }
    setRunning(false);
  }
  ...
  async function testRlsInsertProtection() {
    ...
    if (!otherStore) {
      addResult(true, "Bara en butik i systemet — cross-store test ej möjligt.");
      setRunning(false);
      return;
    }
    const { error } = await supabase.from("tasks").insert({
      title: "[RLS-TEST] Cross-store task",
      category: "Drift",
      priority: "Låg",
    ...
    if (error) {
      ...
      addResult(
        false,
        `VARNING: Cross-store INSERT LYCKADES för butik "${otherStore.name}" — kontrollera RLS!`,
      );
  ...
  async function testRealtimeConnection() {
    ...
    const ch = supabase.channel("testpanel-rt-probe-" + Date.now()).subscribe((status) => {
  ...
  async function testRealtimePing() {
    ...
    const { data, error } = await supabase
      .from("tasks")
      .insert({
        title: "[RT-PING] Realtidspingtest",
        category: "Drift",
        priority: "Låg",
        store_id: activeStore.id,
        status: "todo",
    ...
    if (error) {
      addResult(false, `Ping-insert misslyckades: ${error.message}`);
      setRunning(false);
      return;
    }
    if (data?.id) await supabase.from("tasks").delete().eq("id", data.id);
  ...
  async function checkDataIntegrity() {
    ...
    // Tasks with no store
    const { count: noStore } = await supabase
      .from("tasks")
      .select("id", { count: "exact", head: true })
      .is("store_id", null);
    ...
    const { count: badRec } = await supabase
      .from("tasks")
      .select("id", { count: "exact", head: true })
    ...
    const { count: noStoreInc } = await supabase
      .from("incidents")
      .select("id", { count: "exact", head: true })
      .is("store_id", null);
    ...
    if ((expiredSessions ?? 0) > 0)
      issues.push(`${expiredSessions} utgångna sessioner i databasen`);
  ...
  async function purgeExpiredSessions() {
    if (!window.confirm("Rensa alla utgångna sessioner? Detta kan inte ångras.")) return;
    setRunning(true);
    const { count, error } = await supabase
      .from("app_sessions")
      .delete()
  ...
  async function clearTaskTables() {
    if (
      !window.confirm(
        "Rensa ALLA rader i tasks, steg, frågor, tilldelningar och bilder? Detta kan inte ångras.",
      )
    )
      return;
    setRunning(true);
  ...
  async function clearAuditLog() {
    if (!window.confirm("Rensa hela audit_log? Detta kan inte ångras.")) return;
    setRunning(true);
    const { error } = await supabase
      .from("audit_log")
      .delete()
  ...
  async function clearSystemErrors() {
    if (!window.confirm("Rensa hela system_errors? Detta kan inte ångras.")) return;
    setRunning(true);
    const { error } = await supabase
      .from("system_errors")
      .delete()
  ...
  async function purgeOldAuditLog() {
    if (!window.confirm("Rensa audit-logg äldre än 30 dagar? Detta kan inte ångras.")) return;
    setRunning(true);
  ...
  async function checkScheduleData() {
    ...
    const [imports, shifts, employees, deliveries] = await Promise.all([
      supabase
        ...
        .select("id, week_number, year, filename, store_id")
      ...
      supabase
        .from("delivery_plans")
        .select("id, week_number, year, filename")
        .order("imported_at", { ascending: false })
        .limit(5),
    ]);
    addResult(
      true,
      `Schema: ${shifts.count ?? 0} pass, ${employees.count ?? 0} anställda i ${(imports.data ?? []).length} importer.`,
    );
    (imports.data ?? []).forEach((imp) =>
      addResult(true, `  Import: v${imp.week_number}/${imp.year} — ${imp.filename}`),
    );
    addResult(true, `Leveransplan: ${(deliveries.data ?? []).length} planer.`);
    (deliveries.data ?? []).forEach((d) =>
      addResult(true, `  Leverans: v${d.week_number}/${d.year} — ${d.filename}`),
    );
    setRunning(false);
  }
  ...
  async function deleteAllScheduleImports() {
    ...
    if (
      !window.confirm(
        `Radera alla schemaimporter (inkl. pass) för ${activeStore.name}? Detta kan inte ångras.`,
      )
    )
      return;
    setRunning(true);
    ...
    addResult(
      true,
      `${shiftCount ?? 0} schemaimporter (+ alla pass) raderade för ${activeStore.name}.`,
    );
    await loadStats();
    setRunning(false);
  }
  ...
  async function listInactiveUsers() {
    ...
    if ((data ?? []).length === 0) addResult(true, "Inga inaktiva användare.");
      ...
      (data ?? []).forEach((u) =>
        addResult(
          false,
          `Inaktiv: ${u.display_name} (${u.username}) — roll: ${u.role}, senast inloggad: ${u.last_login ? new Date(u.last_login).toLocaleDateString("sv-SE") : "aldrig"}`,
        ),
      );
    setRunning(false);
  }
  ...
  async function listNeverLoggedIn() {
    ...
    if ((data ?? []).length === 0)
      addResult(true, "Alla aktiva användare har loggat in minst en gång.");
    else
      (data ?? []).forEach((u) =>
        addResult(
          false,
          `Aldrig inloggad: ${u.display_name} (${u.username}) — skapad: ${new Date(u.created_at).toLocaleDateString("sv-SE")}`,
        ),
      );
    setRunning(false);
  }
  ...
  async function listUsersWithoutStore() {
    ...
    if (unlinked.length === 0)
      addResult(true, "Alla aktiva användare är kopplade till minst en butik.");
    else
      unlinked.forEach((u) => addResult(false, `Ingen butik: ${u.display_name} (${u.username})`));
    setRunning(false);
  }
  ...
  async function inspectFile(file: File): Promise<FileInspection> {
    ...
    if (isXml) {
      try {
        ...
        children.forEach((c) =>
          Array.from(c.children).forEach((gc) => {
            grandChildCounts[gc.tagName] = (grandChildCounts[gc.tagName] ?? 0) + 1;
          }),
        );
        ...
        const sampleFields = sampleEl
          ? Array.from(sampleEl.children)
              .slice(0, 15)
              .map((c) => ({ tag: c.tagName, sample: c.textContent?.trim().slice(0, 60) ?? "" }))
          : [];
    ...
    if (isCsv) {
      try {
        ...
        if (lines.length === 0)
          return { name: file.name, type: "csv", encoding, sizeKb, error: "Tom fil." };
    ...
    return {
      ...
      encoding,
      sizeKb,
      error: "Okänt filformat. Endast CSV och XML stöds.",
    };
  }
  ...
  async function deleteTestData() {
    if (!window.confirm("Radera all [TEST]-data (uppgifter + avvikelser)? Detta kan inte ångras."))
      return;
    setRunning(true);
    const { data: testTaskRows } = await supabase
      .from("tasks")
      .select("id")
      .like("title", "[TEST]%");
    ...
    if (errs.length) addResult(false, `Rensning: ${errs.map((e) => e?.message).join("; ")}`);
      ...
      addResult(
        true,
        `Alla [TEST]-uppgifter och [TEST]-avvikelser raderade (${ids.length} uppgifter, bilder inräknade).`,
      );
    await loadStats();
    setRunning(false);
  }
  ...
  async function loadAuditLog() {
    const { data, error } = await supabase
      ...
      .limit(30);
    if (error) {
      addResult(false, `Audit log misslyckades: ${error.message}`);
      return;
    }
  ...
  return (
    <div className="mx-auto max-w-[1100px] px-5 py-8 md:px-8 md:py-10">
      ...
      <div className="mt-2 mb-6 flex items-start gap-2 rounded-xl border border-warning/40 bg-warning/10 px-4 py-3 text-sm text-warning-foreground">
        ...
        <span>
          Denna panel är endast synlig för administratörer. Åtgärder kan skapa och permanent radera
          data i databasen.
        </span>
      </div>
        ...
        <div className="mb-6 grid grid-cols-4 gap-2.5 sm:grid-cols-6 lg:grid-cols-12">
          {statItems.map(({ label, value }) => (
            ...
            >
              ...
              <p className="mt-0.5 text-[10px] leading-tight text-coop-gray-900">{label}</p>
            </div>
          ))}
        </div>
      )}
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <Section icon={Clock} title="Tidssimulering" span2>
          ...
          <div
            className={cn(
              ...
              simulatedOffset > 0
                ...
                : "bg-muted/60 text-coop-gray-900",
            )}
          >
            {simulatedLabel}
          </div>
          <div className="flex flex-wrap items-end gap-2">
            <div className="flex-1 min-w-[80px] space-y-1">
              <Label htmlFor="time-amount" className="text-xs">
                Antal
              </Label>
              <Input
                id="time-amount"
            ...
            <div className="flex-1 min-w-[120px] space-y-1">
              <Label htmlFor="time-unit" className="text-xs">
                Enhet
              </Label>
        ...
        <Section icon={Bell} title="Notiser">
          ...
          <div className="space-y-2">
            ...
            />
            <ActionBtn
              label="Skicka 5 bulk-notiser"
              onClick={testBulkNotifications}
              disabled={running}
            />
            <ActionBtn
              label={`Skicka notis till alla i ${activeStore?.name ?? "aktiv butik"}`}
              onClick={sendNotificationToStore}
              disabled={running || !activeStore}
            />
            <ActionBtn
              label="Visa mina 5 senaste notiser"
              onClick={listMyNotifications}
              disabled={running}
            />
            <ActionBtn
              label="Rensa mina notiser"
              onClick={clearMyNotifications}
              disabled={running}
              danger
            />
            <ActionBtn
              label="Radera alla lästa notiser (alla användare)"
              onClick={clearReadNotificationsAllUsers}
              disabled={running}
              danger
            />
            <ActionBtn
              label="Radera ALLA notiser (alla användare)"
              onClick={clearAllNotificationsAllUsers}
              disabled={running}
              danger
            />
          </div>
          <div className="mt-3 space-y-1.5">
            ...
            <div className="flex gap-2">
              <Input
                id="custom-msg"
                aria-label="Anpassat meddelande"
                placeholder="Notis-text…"
                value={customMsg}
              ...
              <Button
                ...
                disabled={running || !customMsg.trim()}
              >
                Skicka
              </Button>
            </div>
        ...
        <Section icon={BellRing} title="Push & OS-notiser" span2>
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            <ActionBtn
              label="Visa push-status (diagnostik)"
              onClick={checkPushStatus}
              disabled={running}
              variant="default"
            />
            <ActionBtn
              label="Skicka OS-testnotis (till mig)"
              onClick={sendTestOsPush}
              disabled={running}
            />
            <ActionBtn
              label={`Skicka push till ${activeStore?.name ?? "aktiv butik"}`}
              onClick={sendStorePush}
              disabled={running || !activeStore}
            />
            <ActionBtn
              label="Avregistrera denna enhets push"
        ...
        <Section icon={ListChecks} title="Uppgifter">
          ...
          <div className="space-y-2">
            <ActionBtn
              label="Testa skapa + radera"
              onClick={testTaskCreation}
              disabled={running}
              variant="default"
            />
            <ActionBtn
              label="Bulk: skapa 6 uppgifter"
              onClick={bulkCreateTasks}
              disabled={running}
            />
            <ActionBtn
              label="Skapa försenad uppgift"
              onClick={createOverdueTask}
              disabled={running}
            />
            <ActionBtn
              label="Skapa uppgift med steg + frågor"
              onClick={createTaskWithQuestions}
              disabled={running}
            />
            <ActionBtn
              label="Återkommande (använder sim-tid)"
              onClick={createRecurringTask}
              disabled={running}
            />
            <ActionBtn
              label="Radera alla [TEST]-uppgifter"
              onClick={deleteAllTestTasks}
              disabled={running}
              danger
            />
          </div>
        ...
        <Section icon={TriangleAlert} title="Avvikelser">
          ...
          <div className="space-y-2">
            <ActionBtn
              label="Testa skapa + radera"
              onClick={testIncidentCreation}
              disabled={running}
              variant="default"
            />
            <ActionBtn
              label="Skapa eskalerad avvikelse"
              onClick={createEscalatedIncident}
              disabled={running}
            />
            <ActionBtn
              label="Radera alla [TEST]-avvikelser"
              onClick={deleteAllTestIncidents}
              disabled={running}
              danger
            />
            <ActionBtn
              label={`Radera ALLA avvikelser för ${activeStore?.name ?? "aktiv butik"}`}
              onClick={deleteAllIncidentsForStore}
              disabled={running || !activeStore}
              danger
            />
          </div>
        ...
        <Section icon={ImageIcon} title="Bilder & Lagring">
          ...
          <div className="space-y-2">
            <ActionBtn
              label="Testa bildkomprimering (syntetisk bild)"
              onClick={testImageCompression}
              disabled={running}
              variant="default"
            />
            <ActionBtn
              label="Kontrollera herrelösa task-bilder"
              onClick={checkOrphanedTaskImages}
              disabled={running}
            />
            <ActionBtn
              label="Rensa herrelösa task-bilder (DB + Storage)"
              onClick={purgeOrphanedTaskImages}
              disabled={running}
              danger
            />
            <ActionBtn
              label="Kontrollera herrelösa incident-bilder"
              onClick={checkOrphanedIncidentImages}
              disabled={running}
            />
            <ActionBtn
              label="Rensa herrelösa incident-bilder (DB + Storage)"
              onClick={purgeOrphanedIncidentImages}
              disabled={running}
              danger
            />
          </div>
        ...
        >
          ...
          <div className="space-y-2">
            <ActionBtn
              label="Testa WebSocket-anslutning"
              onClick={testRealtimeConnection}
              disabled={running}
              variant="default"
            />
            <ActionBtn
              label="Mät DB-rundtur (insert/delete)"
        ...
        <Section icon={FileText} title="Mallar">
          ...
          <div className="space-y-2">
            <ActionBtn
              label="Hämta mallar (visa detaljer)"
              onClick={testTemplateUsage}
              disabled={running}
              variant="default"
            />
          </div>
        ...
        <Section icon={Shield} title="Säkerhet & RLS">
          ...
          <div className="space-y-2">
            <ActionBtn
              label="Kör RLS SELECT-kontroll (10 tabeller)"
              onClick={testRlsPolicy}
              disabled={running}
              variant="default"
            />
            <ActionBtn
              label="Testa cross-store INSERT-blockering"
              onClick={testRlsInsertProtection}
              disabled={running}
            />
          </div>
        </Section>
        ...
        <Section icon={Users} title="Användare">
          ...
          <div className="space-y-2">
            <ActionBtn
              label="Lista inaktiva användare"
              onClick={listInactiveUsers}
              disabled={running}
              variant="default"
            />
            <ActionBtn
              label="Lista användare som aldrig loggat in"
              onClick={listNeverLoggedIn}
              disabled={running}
            />
            <ActionBtn
              label="Lista användare utan butiksknytning"
              onClick={listUsersWithoutStore}
              disabled={running}
            />
          </div>
        </Section>
        ...
        <Section icon={CalendarDays} title="Schema & Leveransplan">
          ...
          <div className="space-y-2">
            <ActionBtn
              label="Visa senaste 5 importer"
              onClick={checkScheduleData}
              disabled={running}
              variant="default"
            />
            <ActionBtn
              label={`Radera ALLA schemaimporter för ${activeStore?.name ?? "aktiv butik"}`}
              onClick={deleteAllScheduleImports}
              disabled={running || !activeStore}
              danger
            />
          </div>
        ...
        <Section icon={Bug} title="Dataintegritet">
          ...
          <div className="space-y-2">
            <ActionBtn
              label="Kör integritetscheck"
              onClick={checkDataIntegrity}
              disabled={running}
              variant="default"
            />
            <ActionBtn
              label="Rensa utgångna sessioner"
              onClick={purgeExpiredSessions}
              disabled={running}
              danger
            />
            <ActionBtn
              label="Rensa audit-logg äldre än 30 dagar"
              onClick={purgeOldAuditLog}
              disabled={running}
              danger
            />
          </div>
        ...
        <Section icon={Trash2} title="Rensa tabeller">
          ...
          <div className="space-y-2">
            <ActionBtn
              label="Rensa tasks + steps + questions (alla)"
              onClick={clearTaskTables}
              disabled={running}
              danger
            />
            <ActionBtn
              label="Rensa audit_log (alla poster)"
              onClick={clearAuditLog}
              disabled={running}
              danger
            />
            <ActionBtn
              label="Rensa system_errors (alla poster)"
        ...
        <Section icon={FileSearch} title="Filformatsinspektör" span2>
          ...
          >
            <Upload className="mb-2 h-7 w-7 text-coop-gray-900/60" />
            <span className="block text-sm font-medium">Välj en eller flera CSV/XML-filer</span>
            <span className="mt-0.5 block text-xs text-coop-gray-900">
              ...
              {fileInspections.map((ins, i) => (
                <div key={i} className="rounded-xl border border-border/60 overflow-hidden">
                  <div className="flex items-center justify-between gap-2 bg-muted/40 px-4 py-2.5 border-b border-border/60">
                    ...
                    <div className="flex items-center gap-3 text-[10px] text-coop-gray-900">
                      <span>{ins.sizeKb} KB</span>
                      <span
                        className={cn(
                          "rounded-full px-2 py-0.5",
                          ins.encoding.includes("Windows")
                  ...
                  {ins.error ? (
                    ...
                    <div className="p-4 space-y-3">
                      <div>
                        <p className="text-[10px] font-semibold uppercase tabular-nums text-coop-gray-900 mb-1.5">
                      ...
                      {(ins.sampleRows ?? []).length > 0 && (
                        <div>
                          <p className="text-[10px] font-semibold uppercase text-coop-gray-900 mb-1.5">
                            Exempelrader
                          </p>
                              ...
                              <thead>
                                <tr className="border-b border-border/50 bg-muted/40">
                                  {ins.columns?.map((col, ci) => (
                                    <th
                                      key={ci}
                                      className="px-2.5 py-1.5 text-left font-medium text-coop-gray-900 whitespace-nowrap"
                                    >
                                      {col || `(${ci + 1})`}
                                    </th>
                                  ))}
                                ...
                                {ins.sampleRows?.map((row, ri) => (
                                  <tr key={ri} className="hover:bg-muted/20">
                                    {ins.columns?.map((col, ci) => (
                                      <td
                                        key={ci}
                                        className="px-2.5 py-1.5 font-mono text-coop-gray-900 max-w-[200px] truncate"
                                      >
                                        {row[col] || "—"}
                                      </td>
                                    ))}
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {ins.rootTags?.map((tag, ti) => (
                            <span
                              key={ti}
                              className="rounded-full bg-orange-100 text-orange-700 px-2.5 py-0.5 text-[11px] font-medium"
                            >{`<${tag}>`}</span>
                          ))}
                        </div>
                      </div>
                          ...
                          {(ins.sampleFields ?? []).length > 0 && (
                            <div className="overflow-hidden rounded-lg border border-border/50">
                              <table className="w-full text-[11px]">
                                <thead>
                                  <tr className="border-b border-border/50 bg-muted/40">
                                    <th className="px-2.5 py-1.5 text-left font-medium text-coop-gray-900">
                                      Tagg
                                    </th>
                                    <th className="px-2.5 py-1.5 text-left font-medium text-coop-gray-900">
                                      Exempelvärde
                                ...
                                <tbody className="divide-y divide-border/40">
                                  {ins.sampleFields?.map((f, fi) => (
                                    <tr key={fi} className="hover:bg-muted/20">
                                      ...
                                      <td className="px-2.5 py-1.5 text-coop-gray-900">
                                        {f.sample || "—"}
                                      </td>
                                    </tr>
                                  ))}
          ...
          {showAudit && (
            ...
            >
              <div className="border-b border-border/60 px-4 py-2.5 flex items-center justify-between">
                <p className="text-xs font-semibold text-coop-gray-900">Senaste 30 händelser</p>
                <button
              ...
              <div className="max-h-72 overflow-y-auto">
                {auditLog.length === 0 ? (
                  ...
                  auditLog.map((entry) => (
                    <div
                      key={entry.id}
                      className="border-b border-border/40 px-4 py-2.5 last:border-0 hover:bg-muted/30"
                    >
                      <div className="flex items-center justify-between gap-2">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs text-coop-gray-900">{entry.entity}</span>
                          {entry.entity_id && (
                            <span className="font-mono text-[10px] text-coop-gray-900/60">
                              {entry.entity_id.slice(0, 8)}
                            </span>
                          )}
                        </div>
                        <span className="shrink-0 text-[10px] text-coop-gray-900/60">
                      ...
                      {entry.actor && (
                        <p className="mt-0.5 text-[10px] text-coop-gray-900">
      ...
      {results.length > 0 && (
        <div className="mt-6">
          <div className="mb-2 flex items-center justify-between">
            <h3 className="text-sm font-semibold tabular-nums">Resultatlogg ({results.length})</h3>
            <Button
              variant="ghost"
              size="sm"
