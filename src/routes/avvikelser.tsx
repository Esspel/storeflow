...
function IssuesPage() {
  ...
  const fetchQrTokens = async () => {
    ...
    setQrLoading(true);
    const { data } = await supabase.from("qr_tokens")
        ...
        actions={
          <div className="flex gap-2">
            ...
            {isManager && activeStore && (
              <Button variant="outline" className="rounded-full hidden lg:flex" onClick={() => { setShowQrModal(true); fetchQrTokens(); }}>
        ...
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-xl border border-border/60 bg-muted/20 p-4 space-y-3">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Ny QR-kod</p>
              <div className="flex gap-2">
                <input
                  value={qrZoneName}
                ...
                >
                  {["Drift","Kvalitet","Säkerhet","Hygien","Personal","Övrigt"].map(c => <option key={c} value={c}>{c}</option>)}
                </select>
              </div>
              <Button
                size="sm"
