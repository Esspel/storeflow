...
import { AlertCircle, ChevronDown, ChevronUp, Download, Eye, Mail, ShieldCheck, X } from "lucide-react";
...
import {
  supabase, type SupportTicket, logAudit,
  insertSupportTicket, errorToSwedish,
} from "@/lib/supabase";
...
function AdminSupportPage() {
  ...
  const exportCSV = () => {
    const rows = [
      ["ID", "Skapad", "Status", "Användare", "Butik", "App-version", "Kö-längd", "Senaste fel", "Komponenter", "Meddelande"],
      ...tickets.map((t) => [
        t.id,
  ...
  const closeDetail = () => { setDetailTicket(null); setReply(""); };
  ...
  const sendReply = async () => {
    ...
    if (error) { toast.error(errorToSwedish(error)); setReplying(false); return; }
    toast.success("Svar skickat");
    logAudit(user?.id ?? null, "support.reply", "support_tickets", detailTicket!.id, { reply: reply.trim() });
    setReply("");
    setReplying(false);
    fetchTickets();
  ...
  const STATUS_TONE: Record<string, "default" | "destructive" | "outline" | "secondary"> = { open: "destructive", closed: "secondary" };
  const tone = (s: string): "default" | "destructive" | "outline" | "secondary" => STATUS_TONE[s] ?? "secondary";
    ...
    return (
      <div className="flex items-center justify-center h-64 text-center">
        <div className="max-w-md">
          ...
          <p className="mt-1 text-sm text-muted-foreground">Du har inte behörighet att se supportärenden.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-[1200px] px-5 py-8 md:px-8 md:py-10">
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-6">
        <PageHeader title="Supportärenden" description="Hantera inkomna supportärenden från användare" />
          ...
          <DialogContent className="sm:max-w-3xl">
            ...
            <div className="space-y-4 py-4">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div><span className="text-muted-foreground">Skapad:</span> <span className="ml-2 font-mono">{new Date(detailTicket.created_at).toLocaleString("sv-SE")}</span></div>
                <div><span className="text-muted-foreground">Status:</span> <span className="ml-2">{STATUS_LABELS[detailTicket.status] ?? detailTicket.status}</span></div>
                <div><span className="text-muted-foreground">Användare:</span> <span className="ml-2 font-mono">{detailTicket.user_id ?? "—"}</span></div>
                <div><span className="text-muted-foreground">Butik:</span> <span className="ml-2 font-mono">{detailTicket.store_id ?? "—"}</span></div>
                <div><span className="text-muted-foreground">App-version:</span> <span className="ml-2 font-mono">{detailTicket.app_version ?? "—"}</span></div>
                <div><span className="text-muted-foreground">Offline-kö:</span> <span className="ml-2">{detailTicket.offline_queue_length ?? 0}</span></div>
                {detailTicket.components?.length && (
                  <div className="col-span-2"><span className="text-muted-foreground">Komponenter:</span> <span className="ml-2">{detailTicket.components.join(", ")}</span></div>
                )}
                {detailTicket.resolved_at && (
                  <div className="col-span-2"><span className="text-muted-foreground">Löst:</span> <span className="ml-2 font-mono">{new Date(detailTicket.resolved_at).toLocaleString("sv-SE")}</span></div>
                )}
              </div>

              <div>
                <label className="block text-sm font-medium text-muted-foreground mb-1">Meddelande från användare</label>
                <Textarea value={detailTicket.message ?? ""} readOnly className="bg-muted/50" rows={3} />
              </div>

              {detailTicket.last_error && (
                <div>
                  <label className="block text-sm font-medium text-muted-foreground mb-1">Senaste fel</label>
                  <Textarea value={detailTicket.last_error} readOnly className="bg-destructive/5 text-destructive font-mono" rows={2} />
                </div>
              )}

              {detailTicket.idb_usage && (
                <div>
                  <label className="block text-sm font-medium text-muted-foreground mb-1">IndexedDB-användning</label>
                  <Textarea value={detailTicket.idb_usage} readOnly className="bg-muted/50 font-mono" rows={1} />
                </div>
              )}

              <div className="border-t border-border/60 pt-4">
                <label className="block text-sm font-medium text-muted-foreground mb-2">Svara / Kommentera</label>
                <div className="flex gap-2">
                  ...
                  <div className="flex flex-col gap-2">
                    <Button onClick={sendReply} disabled={replying || !reply.trim()} className="rounded-full">
                      {replying ? "Skickar..." : "Skicka"}
                    </Button>
                    {detailTicket.status === "open" ? (
                      <Button variant="outline" onClick={() => resolveTicket(detailTicket.id)} className="rounded-full gap-2">
                        <ShieldCheck className="h-3.5 w-3.5" /> Markera löst
                      </Button>
                    ) : (
                      <Button variant="outline" onClick={() => reopenTicket(detailTicket.id)} className="rounded-full gap-2">
