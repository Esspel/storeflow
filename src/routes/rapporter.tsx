...
function ReportsPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (sessions.length > 0) {
        ...
        const userIds = [
          ...new Set(
            sessions.map((s: { conducted_by: string | null }) => s.conducted_by).filter(Boolean),
          ),
        ] as string[];
        const { data: users } =
          userIds.length > 0
        ...
        const enriched: KundrunSummary[] = sessions.map(
          (s: {
            id: string;
            status: string;
            started_at: string;
            ...
            const sResponses = (responses ?? []).filter(
              (r: { session_id: string }) => r.session_id === s.id,
            );
            return {
              id: s.id,
              status: s.status,
              ...
              zone_defects: sResponses.filter((r: { result: string }) => r.result === "avvikelse")
                .length,
  ...
  const avgDefectsPerRunda =
    completedRundor > 0 ? (totalDefects / completedRundor).toFixed(1) : "0";
  ...
  function exportKundrunCSV() {
    ...
    exportCSV(
      rows,
      `kundrunda-${activeStore?.name ?? "export"}-${new Date().toISOString().slice(0, 10)}.csv`,
    );
  }
  ...
  return (
    <div className="mx-auto max-w-[1400px] px-5 py-8 md:px-8 md:py-10">
      ...
      <div className="mb-8 grid grid-cols-2 gap-4 md:grid-cols-4">
        ...
        <StatCard
          label="Missade uppgifter"
          value={missedTasks}
          tone={missedTasks > 0 ? "destructive" : "default"}
        />
        <StatCard
          ...
          <TabsContent value="tasks" className="mt-6">
            ...
            <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
              {[
                ...
                {
                  label: "Försenade",
                  value: tasks.filter((t) => t.status === "late").length,
          ...
          <TabsContent value="incidents" className="mt-6">
            ...
            <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
              {[
                ...
                {
                  label: "Eskalerad",
                  value: incidents.filter((i) => i.status === "escalated").length,
                  cls: "text-destructive",
                },
                {
                  label: "Löst",
              ...
              {["Låg", "Medel", "Hög", "Kritisk"].map((prio) => (
                ...
                >
                  ...
                  <span className="text-sm font-semibold text-primary">
                    {incidents.filter((i) => i.priority === prio).length}
                  </span>
                </div>
              ))}
            </div>
