    ...
    <div className="relative flex h-20 w-20 items-center justify-center">
      <svg className="-rotate-90" width="80" height="80">
        ...
        <circle
          ...
          className={cn("transition-colors duration-500", color.replace("text-", "stroke-"))}
...
function KundrundaPage() {
  ...
  const [checkpointRefImages, setCheckpointRefImages] = useState<
  ...
  const [deleteCheckpointTarget, setDeleteCheckpointTarget] = useState<{
    checkpoint: KundrundaCheckpoint;
    zoneId: string;
  } | null>(null);
    ...
    const [zonesRes, sessionsRes, defectsRes, localVersionRes] = await Promise.all([
      ...
      (() => {
        let q = supabase
          ...
          .select(
            "*, store:stores(id,name,sap_site_id), conductor:app_users!conducted_by(id,display_name)",
          )
    ...
    if (zonesRes.data) {
      ...
      for (const zone of z) {
        for (const cp of zone.checkpoints) {
          refMap[cp.id] = (cp.images ?? []).map((img) => ({
            id: img.id,
      ...
      for (const resp of Object.values(pending)) {
        if (resp.id && resp.id !== "pending") {
          await supabase
            ...
            .update({
              result: resp.result,
              defect_description: resp.defect_description,
              action_taken: resp.action_taken,
              responsible_user_id: resp.responsible_user_id,
  ...
  const startSession = async (zonesToUse = activeZones) => {
    ...
    const { data } = await supabase
      ...
      .select("*, store:stores(id,name,sap_site_id)")
      .maybeSingle();
    if (data) {
  ...
  const checkVersionMatch = async (): Promise<boolean> => {
    ...
    if (data && data.version !== activeSession.version) {
      toast.error(
        "Ändringarna kunde inte sparas eftersom rundan har uppdaterats av en annan användare. Läs in sidan på nytt.",
        { duration: 8000 },
      );
      return false;
    }
  ...
  const resumeSession = async (session: KundrundaSession) => {
    ...
    (respRes.data ?? []).forEach((r: KundrundaResponse) => {
      map[r.checkpoint_id] = r;
    });
    ...
    const firstIncomplete = activeZones.findIndex(
      (z) => !z.checkpoints.every((c) => map[c.id]?.result),
    );
  ...
  const recordOk = async (checkpoint: KundrundaCheckpoint) => {
    ...
    setResponses((prev) => {
      ...
      if (zoneIdx >= 0) {
        ...
        const allDone = zone.checkpoints.every((c) =>
          c.id === checkpoint.id ? true : updated[c.id]?.result,
        );
        if (allDone) {
          haptic.success();
  ...
  const saveDefect = async () => {
    ...
    if (!defectDialog.defect_description.trim()) {
      haptic.error();
      return;
    }
    if (!defectDialog.action_taken.trim()) {
      haptic.error();
      return;
    }
    if (navigator.onLine && !(await checkVersionMatch())) return;
    ...
    const updatedResponses = {
      ...
      [defectDialog.checkpoint_id]: {
        ...(responses[defectDialog.checkpoint_id] ?? ({} as KundrundaResponse)),
        ...updatedEntry,
      },
    };
    setResponses(updatedResponses);
    ...
    if (defectDialog.defect_description.trim() && responseId) {
      ...
      if (responseId) {
        await supabase
          .from("kundrunda_responses")
          .update({ created_task_id: taskId, incident_id: incident?.id ?? null })
          .eq("id", responseId);
      }
    }
  ...
  const uploadRefPhoto = async (file: File, checkpointId: string) => {
    ...
    if (path) {
      const { data } = await supabase
        .from("kundrunda_checkpoint_images")
        .insert({ checkpoint_id: checkpointId, storage_path: path, uploaded_by: user?.id })
        .select("id")
        .maybeSingle();
      if (data)
        setCheckpointRefImages((p) => ({
          ...p,
          [checkpointId]: [...(p[checkpointId] ?? []), { id: data.id, storage_path: path }],
        }));
    }
  };
  ...
  const deleteRefPhoto = async (checkpointId: string, imgId: string, storagePath: string) => {
    ...
    setCheckpointRefImages((p) => ({
      ...p,
      [checkpointId]: (p[checkpointId] ?? []).filter((i) => i.id !== imgId),
    }));
  };
  ...
  const updateScore = async (currentResponses: ResponseMap) => {
    if (!activeSession) return;
    let okCount = 0;
    for (const r of Object.values(currentResponses)) {
      if (r.result === "ok") okCount++;
    }
    const total = activeZones.reduce((s, z) => s + z.checkpoints.length, 0);
    await supabase
      .from("kundrunda_sessions")
      .update({ total_score: okCount, max_score: total })
      .eq("id", activeSession.id);
  ...
  const bulkDeleteSessions = async () => {
    ...
    for (const id of ids) {
      ...
      logAudit(user?.id ?? null, "kundrunda.session.delete", "kundrunda_sessions", id, {
        bulk: true,
      });
    }
  ...
  const saveZone = async () => {
    ...
    await supabase
      .from("kundrunda_zones")
      .update({ name: editZoneForm.name.trim() })
      .eq("id", editZone.id);
    setEditZone(null);
    await fetchData();
  };
  ...
  const saveCheckpoint = async () => {
    ...
    await supabase
      ...
      .update({
        label: editCheckpointForm.label.trim(),
        description: editCheckpointForm.description.trim() || null,
      })
      .eq("id", editCheckpoint.id);
  ...
  const deleteCheckpoint = async () => {
    ...
    deleteStorageFiles(
      (deleteCheckpointTarget.checkpoint.images ?? [])
    ...
    await supabase
      .from("kundrunda_checkpoint_images")
      .delete()
      .eq("checkpoint_id", deleteCheckpointTarget.checkpoint.id);
    await supabase
      .from("kundrunda_checkpoints")
      .delete()
      .eq("id", deleteCheckpointTarget.checkpoint.id);
  ...
  const applyZoneReorder = async (fromIdx: number, toIdx: number) => {
    ...
    await Promise.all(
      withOrder.map((z, i) =>
        supabase.from("kundrunda_zones").update({ sort_order: i }).eq("id", z.id),
      ),
    );
  };
  ...
  const applyCheckpointReorder = async (zoneId: string, fromIdx: number, toIdx: number) => {
    ...
    await Promise.all(
      withOrder.map((c, i) =>
        supabase.from("kundrunda_checkpoints").update({ sort_order: i }).eq("id", c.id),
      ),
    );
  };
  ...
  const allCheckpoints = editableZones.flatMap((z) =>
    z.checkpoints.map((cp) => ({ ...cp, zoneName: z.name })),
  );
    ...
    try {
      ...
      if (shouldReplace && targetZones.length > 0) {
        ...
        deleteStorageFiles(
          targetZones
            .flatMap((z) =>
              (z.checkpoints ?? []).flatMap((cp) =>
                (cp.images ?? []).map((img) => img.storage_path),
              ),
            )
            .filter(Boolean),
        );
      ...
      for (const [zoneName, zoneRows] of zoneMap) {
        let zoneId: string | null =
          currentZones.find((z) => z.name.toLowerCase().trim() === zoneName.toLowerCase().trim())
            ?.id ?? null;
        if (!zoneId) {
        ...
        for (const row of zoneRows) {
          const exists =
            !shouldReplace &&
            existingZone?.checkpoints.some(
              (c) => c.label.toLowerCase().trim() === row.checkpointLabel.toLowerCase().trim(),
            );
          if (!exists) {
            await supabase.from("kundrunda_checkpoints").insert({
              zone_id: zoneId,
      ...
      <div className="mx-auto max-w-2xl px-5 py-8 space-y-4">
        ...
        <div className="rounded-2xl border border-border/60 bg-card p-6">
          <div className="flex items-center gap-4">
            <div className="h-12 w-12 animate-pulse motion-reduce:animate-none rounded-xl bg-muted shrink-0" />
        ...
        {[1, 2, 3].map((i) => (
          ...
          >
            <div className="h-7 w-7 animate-pulse motion-reduce:animate-none rounded-full bg-muted shrink-0" />
            <div className="flex-1 space-y-1.5">
              <div className="h-3.5 w-2/5 animate-pulse motion-reduce:animate-none rounded-md bg-muted" />
              <div className="h-3 w-1/4 animate-pulse motion-reduce:animate-none rounded-md bg-muted/60" />
            </div>
          </div>
        ))}
      </div>
  ...
  if (view === "active" && activeSession !== null) {
    ...
    const incompleteZones = activeZones.filter(
      (z) => !z.checkpoints.every((c) => responses[c.id]?.result),
    );
        ...
        <div className="sticky top-0 z-20 shrink-0 border-b border-border/60 bg-card px-4 py-3">
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-3 min-w-0">
              <button
                className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-muted hover:bg-muted/70 active:scale-95 transition-all motion-reduce:animate-none"
                ...
                <>
                  ...
                  <div className="text-right">
                    <p className="text-xs font-medium tabular-nums">
                      {answeredCount}
                      <span className="text-muted-foreground">/{totalCheckpoints}</span>
                    </p>
                    <div className="h-1.5 w-16 overflow-hidden rounded-full bg-muted mt-0.5">
                      <div
                        className="h-full rounded-full bg-primary transition-all duration-300 motion-reduce:animate-none"
                        style={{ width: `${sessionPct * 100}%` }}
                      />
                    </div>
                  </div>
                  <Button
                    ...
                    className={cn(
                      ...
                      isAllDone ? "bg-success text-success-foreground hover:bg-success/90" : "",
                    )}
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="mx-auto max-w-2xl px-3 py-3 sm:px-5 sm:py-4 space-y-2">
            {activeZones.map((zone, zoneIdx) => {
              ...
              const zoneDefects = zone.checkpoints.filter(
                (c) => responses[c.id]?.result === "avvikelse",
              ).length;
                          ...
                          >
                              ...
                              <div className="mt-3 pt-2">
                                ...
                                <div className="flex gap-1.5 overflow-x-auto">
                                  {refImages.map((img, idx) => (
                                    <button
                                      key={img.id}
                                      aria-label="Visa referensbild"
                                      className="h-14 w-14 shrink-0 overflow-hidden rounded-lg border border-border/60"
                                      onClick={() => {
                                        setViewerImages(
                                          refImages.map((i) => getPublicUrl(i.storage_path)),
                                        );
                                        setViewerIdx(idx);
                                      }}
                                    >
                            ...
                            {isDefect && (
                              <div className="mt-3 pt-2 space-y-1">
                                ...
                                {resp?.sap_article_id &&
                                  (() => {
                                    const mcUrl = mittCoopUrl(
                                      resp.sap_article_id,
                                      activeSession.store?.sap_site_id ?? null,
                                    );
                                    return mcUrl ? (
                                      <a
                                        href={mcUrl}
                                  ...
                                  <div className="flex gap-1.5 pt-1 overflow-x-auto">
                                    {(responseImages[resp.id] ?? []).map((img, idx) => (
                                      <button
                                        ...
                                        onClick={() => {
                                          setViewerImages(
                                            (responseImages[resp.id!] ?? []).map((i) =>
                                              getPublicUrl(i.storage_path),
                                            ),
                                          );
                                          setViewerIdx(idx);
                                        }}
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <Textarea
                    ...
                    onChange={(e) =>
                      setDefectDialog((p) =>
                        p ? { ...p, defect_description: e.target.value } : null,
                      )
                    }
                ...
                <div className="space-y-1.5">
                  <Label htmlFor="defect-action" className="text-xs">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setDefectDialog((p) => (p ? { ...p, action_taken: e.target.value } : null))
                    }
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    onChange={async (e) => {
                      ...
                      if (f) await uploadDefectPhoto(f);
                      e.target.value = "";
                    }}
                  />
                  <Button
                    ...
                    variant="outline"
                    size="sm"
                    className="rounded-full gap-1.5 text-xs w-full"
                    ...
                    return imgs.length > 0 ? (
                      <div className="flex gap-1.5 overflow-x-auto pt-1">
                        {imgs.map((img) => (
                          <div
                            key={img.id}
                            className="relative h-14 w-14 shrink-0 overflow-hidden rounded-lg border border-border/60"
                          >
                            <img
                              src={getPublicUrl(img.storage_path)}
                              alt=""
                ...
                <div className="space-y-1.5">
                  <Label htmlFor="defect-article" className="text-xs">
                    {defectArticleType === "ean"
                      ? "EAN"
                      : defectArticleType === "bnr"
                        ? "BNR"
                        : "Materialnummer"}
                  </Label>
                  <div className="flex items-center gap-2 rounded-lg border border-input bg-background px-3 py-2">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setDefectDialog((p) =>
                          p ? { ...p, sap_article_id: e.target.value.replace(/\D/g, "") } : null,
                        )
                      }
                      onBlur={(e) => {
                        if (e.target.value.trim()) setDefectArticlePrompt(e.target.value.trim());
                      }}
                      placeholder={
                        defectArticleType === "ean" ? "t.ex. 7310865003294" : "t.ex. 1047133"
                      }
                      inputMode="numeric"
                      pattern="[0-9]*"
                      autoCorrect="off"
                      autoCapitalize="none"
                      spellCheck={false}
                      className="flex-1 border-0 bg-transparent text-sm outline-none focus-visible:ring-2 focus-visible:ring-ring placeholder:text-muted-foreground/40"
                    />
                    <select
                      aria-label="Typ av artikelnummer"
                      value={defectArticleType}
                      onChange={(e) => setDefectArticleType(e.target.value as ArticleIdType)}
                      className="border-0 bg-transparent text-[10px] text-muted-foreground outline-none focus-visible:ring-2 focus-visible:ring-ring cursor-pointer shrink-0"
                    >
                    ...
                    {defectDialog.sap_article_id && (
                      <button
                        type="button"
                        aria-label="Rensa artikelnummer"
                        onClick={() =>
                  ...
                  {defectDialog.sap_article_id &&
                    (() => {
                      const url =
                        defectArticleType === "mat-nr"
                          ? (mittCoopUrl(
                              defectDialog.sap_article_id,
                              activeSession.store?.sap_site_id ?? null,
                            ) ??
                            `https://mittcoop.coop.se/sortiment/articles/${defectDialog.sap_article_id}`)
                          : mittCoopSearchUrl(
                              defectDialog.sap_article_id,
                              activeSession.store?.sap_site_id ?? null,
                            );
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    <button
                      ...
                      aria-expanded={defectUserOpen}
                      aria-controls="defect-user-panel"
                      className="flex h-11 w-full items-center justify-between rounded-lg border border-input bg-background pl-3 pr-8 text-sm"
                    >
                      <span
                        className={
                          defectDialog.responsible_user_id
                      ...
                      >
                        {defectDialog.responsible_user_id
                          ? (storeUsers.find((u) => u.id === defectDialog.responsible_user_id)
                              ?.display_name ?? "Välj person")
                          : "Välj person"}
                      </span>
                    </button>
                    {defectDialog.responsible_user_id && (
                      <button
                        type="button"
                        aria-label="Rensa ansvarig"
                        onClick={(e) => {
                          e.stopPropagation();
                          setDefectDialog((p) => (p ? { ...p, responsible_user_id: "" } : null));
                          setDefectUserOpen(false);
                        }}
                        className="absolute right-1 top-1/2 -translate-y-1/2 flex h-8 w-8 items-center justify-center rounded-full text-muted-foreground hover:text-destructive hover:bg-muted/60"
                      >
                        <X className="h-3.5 w-3.5" />
                      </button>
                    )}
                    {defectUserOpen && (
                      <div
                        id="defect-user-panel"
                        className="absolute top-full left-0 z-50 mt-1 w-full rounded-lg border border-border/60 bg-card shadow-lg"
                      >
                        <div className="p-2 border-b border-border/40">
                          <div className="relative">
                            ...
                            <input
                              ...
                              className="w-full h-8 rounded-md border border-border/60 bg-background pl-8 pr-2 text-sm outline-none focus-visible:ring-2 focus-visible:ring-ring"
                            />
                          </div>
                        </div>
                          ...
                          {storeUsers
                            .filter(
                              (u) =>
                                ...
                                u.display_name
                                  .toLowerCase()
                                  .includes(defectUserSearch.toLowerCase()),
                            )
                            .map((u) => (
                              <button
                                key={u.id}
                                ...
                                onClick={() => {
                                  setDefectDialog((p) =>
                                    p ? { ...p, responsible_user_id: u.id } : null,
                                  );
                                  setDefectUserOpen(false);
                                  setDefectUserSearch("");
                                }}
                                className={cn(
                                  "w-full rounded px-2 py-1.5 text-left text-sm hover:bg-muted/50",
                                  defectDialog.responsible_user_id === u.id &&
                ...
                {(!defectDialog.defect_description.trim() || !defectDialog.action_taken.trim()) &&
                  !savingDefect &&
                  (defectDialog.defect_description.length > 0 ||
                    defectDialog.action_taken.length > 0) && (
                    <p role="alert" className="text-xs text-destructive">
                      Beskrivning och åtgärd är obligatoriska fält.
                    </p>
                  )}
                <div className="flex justify-end gap-2 pt-1">
                  ...
                  <Button
                    ...
                    disabled={
                      savingDefect ||
                      !defectDialog?.defect_description.trim() ||
                      !defectDialog?.action_taken.trim()
                    }
                    aria-busy={savingDefect}
                    onClick={saveDefect}
                  >
                    {savingDefect ? (
                      <>
        ...
        <AlertDialog open={showFinishWarning} onOpenChange={setShowFinishWarning}>
          <AlertDialogContent>
            ...
            <AlertDialogFooter className="flex-col gap-2 sm:flex-row">
              ...
              <Button
                variant="outline"
                className="rounded-full w-full sm:w-auto hidden sm:flex"
      ...
      <div className="mx-auto max-w-3xl px-5 py-8 md:px-8 md:py-10">
        <div className="mb-6 flex items-center gap-3">
          <button
            ...
            aria-label="Tillbaka"
            className="flex h-9 w-9 items-center justify-center rounded-xl border border-border/60 bg-card hover:bg-muted/40"
          >
          ...
          <div className="ml-auto flex flex-wrap gap-2">
            ...
            <Button
              ...
              size="sm"
              className="hidden sm:flex rounded-full gap-1.5"
              onClick={() => exportTemplateCsv(displayZones)}
            >
            ...
            <div className="mb-4 flex items-start gap-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 dark:border-amber-800/40 dark:bg-amber-950/20">
              ...
              <div className="flex gap-2 shrink-0">
                ...
                <Button
                  ...
                  onClick={() => setShowDefectsMergeDialog(true)}
                >
          ...
          options={[
            {
              ...
              description:
                "Tar bort alla nuvarande zoner och kontrollpunkter i valt scope och ersätter med CSV-filens innehåll",
              defaultValue: false,
            },
            ...(canEditGlobal
              ? [
                  {
                    ...
                    description:
                      "Välj om importen ska gälla HK-versionen eller den aktiva butikens lokala version",
                    options: [
        ...
        <div className="space-y-4">
          {displayZones.map((zone, zoneIdx) => (
            <div key={zone.id}>
              ...
              <div
                className={cn(
                  "rounded-2xl border border-border/60 bg-card overflow-hidden transition-opacity",
                  dragZoneIdx === zoneIdx && "opacity-40",
                  dropZoneIdx === zoneIdx && dragZoneIdx !== zoneIdx && "ring-2 ring-primary/40",
                )}
                draggable
                onDragStart={(e) => {
                ...
                <div className="flex items-center justify-between gap-2 border-b border-border/40 bg-muted/20 px-4 py-3">
                  ...
                  <div className="flex gap-1">
                    <button
                      ...
                      className="flex h-7 w-7 items-center justify-center rounded-full text-muted-foreground hover:bg-muted/60 hover:text-primary"
                      onClick={() => {
                        setEditZone(zone);
                    ...
                    <button
                      ...
                      className="flex h-7 w-7 items-center justify-center rounded-full text-muted-foreground hover:bg-muted/60 hover:text-destructive"
                ...
                <div className="divide-y divide-border/40">
                  {zone.checkpoints.map((cp, cpIdx) => {
                    ...
                    return (
                      <div key={cp.id}>
                        ...
                        <div
                          className={cn(
                            "px-4 py-3 space-y-2 transition-opacity",
                            isDraggingCp && "opacity-40",
                            dragCpKey?.zoneId === zone.id &&
                              dropCpIdx === cpIdx &&
                              dragCpKey.idx !== cpIdx &&
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2">
                            {refs.map((img) => (
                              <div
                                key={img.id}
                                className="group relative h-14 w-14 shrink-0 overflow-hidden rounded-lg border border-border/60"
                              >
                                <img
                                  src={getPublicUrl(img.storage_path)}
                                  alt=""
                                ...
                                />
                                <button
                                  aria-label="Ta bort referensbild"
                                  className="absolute inset-0 flex items-center justify-center bg-black/40 opacity-60 group-hover:opacity-100 transition-opacity"
                                  onClick={() => deleteRefPhoto(cp.id, img.id, img.storage_path)}
                                >
                  ...
                  <div className="px-4 py-3 space-y-2">
                    ...
                    <Button
                      ...
                      disabled={!newCheckpointLabel.trim()}
        ...
        <input
          ...
          onChange={async (e) => {
            const f = e.target.files?.[0];
            if (f && pendingRefCheckpointId) await uploadRefPhoto(f, pendingRefCheckpointId);
            e.target.value = "";
            setPendingRefCheckpointId(null);
          }}
        />
        ...
        <Dialog
          open={!!editZone}
          onOpenChange={(o) => {
            if (!o) setEditZone(null);
          }}
        >
          <DialogContent className="max-w-sm">
            <DialogHeader>
              <DialogTitle>Redigera zon</DialogTitle>
            </DialogHeader>
            <div className="space-y-3">
              <Input
                aria-label="Zonnamn"
                value={editZoneForm.name}
                onChange={(e) => setEditZoneForm((p) => ({ ...p, name: e.target.value }))}
                className="text-sm"
              />
              <div className="flex justify-end gap-2">
                ...
                <Button size="sm" className="rounded-full" onClick={saveZone}>
                  Spara
                </Button>
              </div>
            </div>
        ...
        >
          <DialogContent className="max-w-sm">
            ...
            <div className="space-y-3">
              <div className="space-y-1.5">
                <Label htmlFor="edit-checkpoint-label" className="text-xs">
                  Etikett
                </Label>
                <Input
                  id="edit-checkpoint-label"
                  value={editCheckpointForm.label}
                  onChange={(e) => setEditCheckpointForm((p) => ({ ...p, label: e.target.value }))}
                  className="text-sm"
                />
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="edit-checkpoint-desc" className="text-xs">
                  Beskrivning (vad ska kontrolleras)
                </Label>
                <Textarea
                  id="edit-checkpoint-desc"
                  value={editCheckpointForm.description}
                  onChange={(e) =>
                    setEditCheckpointForm((p) => ({ ...p, description: e.target.value }))
                  }
                  rows={3}
                  className="resize-none text-sm"
                />
              ...
              <div className="flex justify-end gap-2">
                <Button
                  ...
                  size="sm"
                  className="rounded-full"
                  onClick={() => setEditCheckpoint(null)}
                >
                  Avbryt
                </Button>
                <Button size="sm" className="rounded-full" onClick={saveCheckpoint}>
                  Spara
                </Button>
              </div>
            </div>
        ...
        <AlertDialog
          ...
          onOpenChange={(o) => !o && setDeleteZoneTarget(null)}
        >
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>Ta bort zon</AlertDialogTitle>
              <AlertDialogDescription>
                Ta bort <strong>{deleteZoneTarget?.name}</strong> och alla dess kontrollpunkter?
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
        ...
        >
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Ta bort <strong>{deleteCheckpointTarget?.checkpoint.label}</strong>?
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              <AlertDialogCancel>Avbryt</AlertDialogCancel>
        ...
        <AlertDialog
          open={showDefectsMergeDialog}
          onOpenChange={(o) => !o && setShowDefectsMergeDialog(false)}
        >
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>Slå ihop avvikelser med HK</AlertDialogTitle>
              <AlertDialogDescription>
                HKs nya avvikelser läggs till i din butiks lista. Befintliga lokala avvikelser
                påverkas inte. Länkade avvikelser vars text ändrats hos HK uppdateras.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              <AlertDialogCancel>Avbryt</AlertDialogCancel>
  ...
  return (
    <div className="mx-auto max-w-2xl px-5 py-8 md:px-8 md:py-10">
      <PageHeader
        title="Kundrunda"
        description={
          activeStore ? `Butiksrond för ${activeStore.name}` : "Digital butiksinspektion."
        }
        actions={
          <div className="flex items-center gap-2">
      ...
      {localVersion?.central_version_pending && (
        <div className="mb-4 rounded-2xl border border-warning/40 bg-warning/10 p-4">
          <div className="flex items-start gap-3">
            ...
            <Button
              size="sm"
              className="rounded-full shrink-0 bg-warning/20 text-warning-foreground hover:bg-warning/30 border-0"
              onClick={() => setShowVersionChoiceDialog(true)}
            >
              Hantera
            </Button>
          </div>
          ...
          <div className="divide-y divide-border">
            {inProgressSessions.map((s) => (
              <div key={s.id}>
                <button
                  type="button"
                  aria-expanded={priorSessionAction?.id === s.id}
                  aria-controls={`prior-session-panel-${s.id}`}
                ...
                {priorSessionAction?.id === s.id && (
                  <div id={`prior-session-panel-${s.id}`} className="flex gap-2 px-5 pb-3">
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => {
                        setPriorSessionAction(null);
                        resumeSession(s);
                      }}
                    >
                      Fortsatt runda
        ...
        {inProgressSession && inProgressSessions.length === 1 ? (
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  ...
                  {(
                    inProgressSession as KundrundaSession & { conductor?: { display_name: string } }
                  ).conductor?.display_name && (
                    <span className="ml-1">
                      ·{" "}
                      {
                        (
                          inProgressSession as KundrundaSession & {
                            conductor?: { display_name: string };
                          }
                        ).conductor!.display_name
                      }
                    </span>
            ...
            <div className="flex flex-col gap-2 sm:flex-row">
              {isManager && (
                <button
                  className="flex h-8 w-8 items-center justify-center rounded-full text-muted-foreground hover:bg-muted/60 hover:text-destructive"
              ...
              <Button
                size="sm"
                variant="outline"
                className="rounded-full gap-1.5 shrink-0 text-xs"
              ...
              <Button
                ...
                onClick={() => resumeSession(inProgressSession)}
              >
            ...
            {completedSessions.slice(0, 10).map((s) => {
              ...
              return (
                <div
                  key={s.id}
                  className={cn(
                    "group flex items-center gap-3 rounded-2xl border bg-card px-4 py-4 transition-colors",
                ...
                >
                  ...
                  {canDelete && (
                    <button
                      className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full border border-border/60 text-muted-foreground transition-colors hover:bg-destructive/10 hover:text-destructive"
      ...
      <AlertDialog
        ...
        onOpenChange={(o) => !o && setDeleteSessionTarget(null)}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
      ...
      {/* Bulk delete sessions confirm */}
      <AlertDialog
        open={showBulkDeleteConfirm}
        onOpenChange={(o) => !o && setShowBulkDeleteConfirm(false)}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Ta bort {selectedSessionIds.size} rundor?</AlertDialogTitle>
            <AlertDialogDescription>
              Alla svar och bilder för de valda rundorna tas bort permanent. Detta kan inte ångras.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Avbryt</AlertDialogCancel>
      ...
      <Dialog
        open={showVersionChoiceDialog}
        onOpenChange={(o) => {
          if (!o) setShowVersionChoiceDialog(false);
        }}
      >
        <DialogContent className="max-w-sm">
          <DialogHeader>
          ...
          <div className="flex justify-end mt-2">
            <Button
              ...
              size="sm"
              className="rounded-full"
              onClick={() => setShowVersionChoiceDialog(false)}
            >
              Bestäm senare
            </Button>
          </div>
      ...
      <Dialog
        open={showParallelChoiceDialog}
        onOpenChange={(o) => {
          if (!o) setShowParallelChoiceDialog(false);
        }}
      >
        <DialogContent className="max-w-sm">
          <DialogHeader>
          ...
          <div className="space-y-2 mt-2">
            {[
              {
                choice: "central" as const,
                icon: Upload,
                iconClass: "text-primary",
                title: "Central version",
                desc: "Använd HK:s mall för denna runda.",
              },
              {
          ...
          <div className="flex justify-end mt-2">
            <Button
              ...
              size="sm"
              className="rounded-full"
              onClick={() => setShowParallelChoiceDialog(false)}
            >
              Avbryt
            </Button>
          </div>
      ...
      <AlertDialog
        open={!!defectArticlePrompt}
        onOpenChange={(o) => {
          if (!o) setDefectArticlePrompt(null);
        }}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              Vad är <span className="font-mono">{defectArticlePrompt}</span>?
            </AlertDialogTitle>
            <AlertDialogDescription>
          ...
          <AlertDialogFooter className="flex-col gap-2 sm:flex-row">
            {(["mat-nr", "ean", "bnr"] as ArticleIdType[]).map((t) => (
              <AlertDialogAction
                key={t}
                onClick={() => {
                  setDefectArticleType(t);
                  setDefectArticlePrompt(null);
                }}
              >
                {t === "mat-nr"
                  ? "Materialnummer"
