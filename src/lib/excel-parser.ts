    ...
    reader.onload = (e) => {
      try {
        ...
        const targetSheetName =
          workbook.SheetNames.find((n) => n.toLowerCase().includes("sapui5")) ||
          workbook.SheetNames[0];
        const targetWorksheet = workbook.Sheets[targetSheetName];

        // Convert to JSON with header row as array
        const jsonData = XLSX.utils.sheet_to_json<Record<string, unknown>>(targetWorksheet, {
        ...
        const dataRows = (
          jsonData.slice(1) as unknown as (string | number | null | undefined)[][]
        ...
        const rows: DeliveryNoteRow[] = dataRows.map((row) => {
          ...
          return {
            ...
            varumärke:
              paddedRow[SAPUI5_EXPORT_COLUMN_MAP.brand] !== ""
            ...
            beställningskvantitet:
              paddedRow[SAPUI5_EXPORT_COLUMN_MAP.orderQuantity] !== ""
