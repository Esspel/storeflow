...
export function mittCoopUrl(
  ...
  let url = `https://mittcoop.coop.se/sortiment/articles/${encodeURIComponent(sapArticleId.trim())}?siteId=${encodeURIComponent(sapSiteId.trim())}`;
...
export function mittCoopSearchUrl(
  ...
  let url = `https://mittcoop.coop.se/sortiment/artiklar?siteId=${encodeURIComponent(sapSiteId.trim())}&search=${encodeURIComponent(search.trim())}`;
  if (opts?.categoryId) url += `&categoryIds=${opts.categoryId}`;
  if (opts?.statusCode) url += `&statusCodes=${opts.statusCode}`;
  return url;
}
