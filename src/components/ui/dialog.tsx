...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Overlay
    ref={ref}
    className={cn(
      "fixed inset-0 z-50 bg-coop-svart/80 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 backdrop-blur-sm",
      className,
    )}
    {...props}
  />
...
>(({ className, children, hideCloseButton = false, ...props }, ref) => {
  // p-0-dialoger hanterar själva sin scroll + padding (t.ex. "flex flex-col p-0 overflow-hidden")
  // och ska inte få ytterligare padding/scroll ovanpå det (dubbel padding + nästlad scroll).
  const fullBleed = /\bp-0\b/.test(className ?? "");
  return (
    <DialogPortal>
      <DialogOverlay />
      ...
      >
        ...
        {fullBleed ? (
          /* Innehållet äger sin egen layout/scroll — bara en flex-container här */
...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Title
    ref={ref}
    className={cn("text-lg font-semibold leading-none tracking-tight text-coop-gray-900", className)}
    {...props}
  />
));
