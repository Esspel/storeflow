      ...
      <DialogContent hideCloseButton className="w-full sm:max-w-lg p-0 gap-0 overflow-hidden">
        ...
        <div className="flex items-center justify-between border-b border-border/60 px-4 py-3 sm:px-5 sm:py-4">
          <div className="min-w-0 pr-2">
            ...
            {description && <p className="mt-0.5 text-xs text-coop-gray-900">{description}</p>}
          </div>
          <button
            onClick={onClose}
            className="shrink-0 rounded-full p-1 text-coop-gray-900 hover:text-coop-gray-900 transition-colors"
            aria-label="Stäng"
          >
            <X className="h-4 w-4" />
          </button>
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-4">
            ...
            {selectedFile ? (
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  className="ml-2 rounded-full p-1 text-coop-gray-900 hover:text-destructive transition-colors"
                >
              ...
              <>
                <div className="flex h-12 w-12 items-center justify-center rounded-full bg-muted mb-3">
                  <Upload className="h-5 w-5 text-coop-gray-900" />
                </div>
                ...
                <p className="mt-1.5 text-[10px] text-coop-gray-900/60">
              ...
              {options.map((opt) => {
                ...
                if (opt.type === "section-header") {
                  return (
                    <p
                      key={opt.key}
                      className="text-[10px] font-semibold uppercase tracking-wider text-coop-gray-900/70 pt-1 first:pt-0"
                    >
                      {opt.label}
                    </p>
                  );
                  ...
                  return (
                    <label key={opt.key} className="flex cursor-pointer items-start gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        {opt.description && (
                          <p className="mt-0.5 text-xs text-coop-gray-900">{opt.description}</p>
                        )}
                      </div>
                    </label>
                  );
                    ...
                    <div key={opt.key} className="space-y-1.5">
                      <div className="flex items-center justify-between gap-3">
                        <div className="min-w-0">
                          ...
                          {opt.description && (
                            <p className="text-xs text-coop-gray-900">{opt.description}</p>
                          )}
                        </div>
                        <Select
          ...
          <div className="flex items-center justify-end gap-2 pt-1">
            <Button variant="ghost" size="sm" onClick={onClose} className="text-coop-gray-900">
              Avbryt
            </Button>
            <Button
              size="sm"
