...
export function PageHeader({
  ...
  return (
    <div className="mb-6 flex flex-col gap-3 md:mb-8 md:flex-row md:items-end md:justify-between">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight md:text-3xl coop-font-heading-sm">
          {title}
        </h1>
        {description && (
          <p className="mt-1 text-sm text-coop-gray-900 coop-font-body">{description}</p>
        )}
      </div>
  ...
  return (
    <div className="group relative overflow-hidden rounded-2xl border border-border/60 bg-coop-gray-100 p-5 shadow-[var(--shadow-sm)] transition-shadow hover:shadow-[var(--shadow-md)]">
      <div className="flex items-start justify-between">
        <div>
          <p className="text-xs font-medium uppercase tracking-wide text-coop-gray-900">{label}</p>
          <p className="mt-2 text-3xl font-semibold tracking-tight tabular-nums coop-font-heading">
            {value}
          </p>
          {(delta || hint) && (
